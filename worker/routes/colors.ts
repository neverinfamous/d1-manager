...
export async function handleColorRoutes(
  ...
  if (request.method === "GET" && url.pathname === "/api/databases/colors") {
    logInfo("Getting all database colors", {
      module: "colors",
      operation: "list",
    });
    ...
    try {
      const result = await db
        .prepare("SELECT database_id, color FROM database_colors")
      ...
      logWarning(
        `Failed to get colors: ${err instanceof Error ? err.message : String(err)}`,
  ...
  if (
    request.method === "PUT" &&
    /^\/api\/databases\/[^/]+\/color$/.exec(url.pathname)
  ) {
    const dbId = url.pathname.split("/")[3];
    logInfo(`Updating color for database: ${dbId ?? "unknown"}`, {
      module: "colors",
      operation: "update",
      ...(dbId !== undefined && { databaseId: dbId }),
    });
    ...
    try {
      if (color === null) {
        // Remove color
        await db
          .prepare("DELETE FROM database_colors WHERE database_id = ?")
          .bind(dbId)
          .run();
      } else {
        // Upsert color
      ...
      if (errorMessage.includes("no such table: database_colors")) {
        return new Response(
          JSON.stringify({
            ...
            message:
              "The database colors feature requires a schema update. Please run: npx wrangler d1 execute d1-manager-metadata --remote --file=worker/migrations/002_add_database_colors.sql",
  ...
  if (
    request.method === "GET" &&
    /^\/api\/tables\/[^/]+\/colors$/.exec(url.pathname)
  ) {
    const dbId = url.pathname.split("/")[3];
    logInfo(`Getting table colors for database: ${dbId ?? "unknown"}`, {
      module: "colors",
    ...
    try {
      ...
      logWarning(
        `Failed to get table colors: ${err instanceof Error ? err.message : String(err)}`,
        {
          module: "colors",
          operation: "table_list",
          ...(dbId !== undefined && { databaseId: dbId }),
  ...
  if (
    request.method === "PUT" &&
    /^\/api\/tables\/[^/]+\/[^/]+\/color$/.exec(url.pathname)
  ) {
    const pathParts = url.pathname.split("/");
    const dbId = pathParts[3];
    const tableName = decodeURIComponent(pathParts[4] ?? "");
    logInfo(
      `Updating color for table: ${tableName} in database: ${dbId ?? "unknown"}`,
      {
        module: "colors",
    ...
    try {
      if (color === null) {
        ...
        await db
          ...
          .bind(dbId, tableName, color, userEmail)
          .run();
      }
      ...
      logWarning(
        `Failed to update table color: ${err instanceof Error ? err.message : String(err)}`,
        {
          module: "colors",
          operation: "table_update",
          ...(dbId !== undefined && { databaseId: dbId }),
      ...
      if (errorMessage.includes("no such table: table_colors")) {
        return new Response(
          JSON.stringify({
            error: "Database upgrade required",
            message:
              "The table colors feature requires a schema update. Please run: npx wrangler d1 execute d1-manager-metadata --remote --file=worker/migrations/003_add_table_colors.sql",
            requiresUpgrade: true,
            success: false,
          }),
          {
