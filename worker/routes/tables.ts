...
import {
  triggerWebhooks,
  createTableCreatePayload,
  createTableDeletePayload,
  createTableUpdatePayload,
  createBulkDeleteCompletePayload,
} from "../utils/webhooks";
...
export async function handleTableRoutes(
  ...
  try {
    ...
    if (
      request.method === "GET" &&
      url.pathname === `/api/tables/${dbId}/list`
    ) {
      logInfo(`Listing tables for database: ${dbId}`, {
        module: "tables",
        operation: "list",
        databaseId: dbId,
      });
      ...
      const tables = (result.results as TableInfo[]).filter(
        (table: TableInfo) =>
      ...
      if (tablesToCount.length > 0) {
        ...
        const countPromises = tablesToCount.map(async (table) => {
          try {
            ...
            const countResult = await executeQueryViaAPI(
              dbId,
              `SELECT COUNT(*) as count FROM "${sanitizedName}"`,
              env,
            );
            const countRow = countResult.results[0] as
              | { count: number }
              | undefined;
    ...
    if (
      request.method === "GET" &&
      /^\/api\/tables\/[^/]+\/schema\/[^/]+$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[5] ?? "");
      logInfo(`Getting schema for table: ${tableName}`, {
        module: "tables",
        operation: "schema",
        databaseId: dbId,
        metadata: { tableName },
      ...
      try {
        ...
        const indexes = indexResult.results as {
          name: string;
          unique: number;
          origin: string;
          partial: number;
        ...
        for (const idx of indexes) {
          if (idx.unique === 1) {
            ...
            const idxInfoResult = await executeQueryViaAPI(
              dbId,
              idxInfoQuery,
              env,
            );
      ...
      const enhancedResults = (xinfoResult.results as XInfoColumn[]).map(
        (col) => ({
          ...col,
          unique: uniqueColumns.has(col.name) || undefined,
          // Map hidden values: 2=virtual generated, 3=stored generated
          generatedExpression:
            col.hidden === 2 || col.hidden === 3 ? "(computed)" : undefined,
        }),
      );
    ...
    if (
      request.method === "GET" &&
      /^\/api\/tables\/[^/]+\/data\/[^/]+$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[5] ?? "");
      ...
      for (const [key, value] of url.searchParams.entries()) {
        if (key.startsWith("filter_")) {
          ...
          const filterValue2 = url.searchParams.get(
            `filterValue2_${columnName}`,
          );
          const filterValues = url.searchParams.get(
            `filterValues_${columnName}`,
          );
      ...
      logInfo(`Getting data for table: ${tableName}`, {
        module: "tables",
        operation: "data",
        databaseId: dbId,
        metadata: {
      ...
      if (Object.keys(filters).length > 0) {
        ...
        for (const [columnName, filter] of Object.entries(filters)) {
          ...
          if (filter.logicOperator !== undefined)
            condition.logicOperator = filter.logicOperator;
          filterConditions[columnName] = condition;
        }

        const { whereClause: clause } = buildWhereClause(
          filterConditions,
          schema,
        );
        whereClause = clause;
    ...
    if (
      request.method === "GET" &&
      /^\/api\/tables\/[^/]+\/indexes\/[^/]+$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[5] ?? "");
      logInfo(`Getting indexes for table: ${tableName}`, {
        module: "tables",
        operation: "indexes",
        databaseId: dbId,
        metadata: { tableName },
    ...
    if (
      request.method === "GET" &&
      /^\/api\/tables\/[^/]+\/foreign-keys\/[^/]+$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[5] ?? "");
      logInfo(`Getting foreign keys for table: ${tableName}`, {
        module: "tables",
    ...
    if (
      ...
      /^\/api\/tables\/[^/]+\/[^/]+$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[4] ?? "");
      logInfo(`Deleting table: ${tableName}`, {
        module: "tables",
        operation: "delete",
        databaseId: dbId,
        metadata: { tableName },
      ...
      try {
        ...
        try {
          ...
          void logError(
            env,
            snapshotErr instanceof Error ? snapshotErr : String(snapshotErr),
            {
              module: "tables",
          ...
          if (errMsg.includes("Currently processing a long-running export")) {
            logInfo("Database locked by export - cannot delete table", {
              module: "tables",
              operation: "delete",
              databaseId: dbId,
              metadata: { tableName },
            ...
            await finishJobTracking(
              ...
              isLocalDev,
              {
                processedItems: 0,
                errorCount: 1,
                errorMessage: "Database is temporarily locked",
              },
            );
            return new Response(
              JSON.stringify({
                ...
                message:
                  "The database is currently processing an export. Please wait a few minutes and try again.",
              }),
              {
                status: 503,
                headers: jsonHeaders(corsHeaders),
    ...
    if (
      ...
      /^\/api\/tables\/[^/]+\/[^/]+\/rename$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[4] ?? "");
      logInfo(`Renaming table: ${tableName}`, {
        module: "tables",
        operation: "rename",
        databaseId: dbId,
        metadata: { tableName },
    ...
    if (
      request.method === "POST" &&
      /^\/api\/tables\/[^/]+\/[^/]+\/clone$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[4] ?? "");
      logInfo(`Cloning table: ${tableName}`, {
        module: "tables",
        operation: "clone",
        databaseId: dbId,
        metadata: { tableName },
      ...
      try {
        ...
        const schemaResult = await executeQueryViaAPI(
          dbId,
          `PRAGMA table_info("${sanitizedOldTable}")`,
          env,
        );
        ...
        const indexesResult = await executeQueryViaAPI(
          dbId,
          `PRAGMA index_list("${sanitizedOldTable}")`,
          env,
        );
        ...
        for (const index of indexes) {
          if (index.origin === "c") {
            // Only copy user-created indexes
            const indexInfoResult = await executeQueryViaAPI(
              dbId,
              `PRAGMA index_info("${index.name}")`,
              env,
            );
            const indexColumns = indexInfoResult.results as {
              seqno: number;
              cid: number;
              name: string;
            }[];
    ...
    if (
      request.method === "GET" &&
      /^\/api\/tables\/[^/]+\/[^/]+\/strict-check$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[4] ?? "");
      logInfo(`Checking STRICT mode compatibility: ${tableName}`, {
        module: "tables",
      ...
      try {
        ...
        const createSqlResult = await executeQueryViaAPI(
          dbId,
          `SELECT sql FROM sqlite_master WHERE type='table' AND name='${sanitizedTable}'`,
          env,
        );
        const createSql =
          (createSqlResult.results[0] as { sql: string } | undefined)?.sql ??
          "";
        const isVirtualTable =
          createSql.toLowerCase().includes("virtual table") ||
        ...
        const xinfoResult = await executeQueryViaAPI(
          dbId,
          `PRAGMA table_xinfo("${sanitizedTable}")`,
          env,
        );
        ...
        const fkResult = await executeQueryViaAPI(
          dbId,
          `PRAGMA foreign_key_list("${sanitizedTable}")`,
          env,
        );
        ...
        if (isVirtualTable) {
          blockers.push(
            "Virtual tables (FTS5, FTS4, rtree) cannot be converted to STRICT mode",
          );
        }
        ...
        if (generatedColumns.length > 0) {
          ...
          blockers.push(
            `Table has generated columns (${colNames}) which cannot be automatically converted`,
          );
        }

        if (fkSummary.length > 0) {
          warnings.push(
            `Table has ${fkSummary.length} foreign key constraint(s) which will be preserved during conversion`,
          );
        }
    ...
    if (
      request.method === "POST" &&
      /^\/api\/tables\/[^/]+\/[^/]+\/strict$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[4] ?? "");
      logInfo(`Converting table to STRICT: ${tableName}`, {
        module: "tables",
        operation: "strict",
        databaseId: dbId,
        metadata: { tableName },
      ...
      try {
        ...
        if (isVirtualTable) {
          ...
          return new Response(
            JSON.stringify({
              error:
                "Cannot convert to STRICT mode: Virtual tables (like FTS5 full-text search tables) cannot use STRICT mode.",
              details:
                "STRICT mode is only available for regular tables. Virtual tables have their own type system.",
            }),
            {
              status: 400,
              headers: jsonHeaders(corsHeaders),
        ...
        const generatedColumns = columns.filter(
          (col) => col.hidden === 2 || col.hidden === 3,
        );
        if (generatedColumns.length > 0) {
          ...
          return new Response(
            JSON.stringify({
              ...
              details:
                "Tables with generated (computed) columns cannot be automatically converted to STRICT mode. You would need to manually recreate the table with the generated column expressions and the STRICT keyword.",
            }),
            {
              status: 400,
              headers: jsonHeaders(corsHeaders),
        ...
        try {
          ...
          logWarning(
            `Failed to capture undo snapshot before STRICT conversion: ${snapshotErr instanceof Error ? snapshotErr.message : String(snapshotErr)}`,
            {
              module: "tables",
              operation: "snapshot_capture",
              databaseId: dbId,
        ...
        const sampleDataResult = await executeQueryViaAPI(
          dbId,
          `SELECT * FROM "${sanitizedTable}" LIMIT 100`,
          env,
        );
        const sampleRows = sampleDataResult.results as Record<
          string,
          unknown
        >[];
        ...
        const analyzeColumnType = (
          ...
          if (actualTypes.size > 1) {
            ...
            if (
              actualTypes.size === 2 &&
              actualTypes.has("INTEGER") &&
              actualTypes.has("REAL")
            ) {
              return "REAL";
            }
            return "ANY";
        ...
        const formatDefaultValue = (dfltValue: string): string => {
          ...
          // TRUE/FALSE
          if (
            trimmed.toUpperCase() === "TRUE" ||
            trimmed.toUpperCase() === "FALSE"
          ) {
            return trimmed;
          }
        ...
        logInfo(`Creating STRICT table with query: ${createTableQuery}`, {
          module: "tables",
          operation: "strict",
          databaseId: dbId,
        });
        ...
        try {
          ...
          await executeQueryViaAPI(
            dbId,
            `DROP TABLE IF EXISTS "${sanitizedTempTable}"`,
            env,
          );
          const errorMsg =
          ...
          return new Response(
            JSON.stringify({
              ...
              details:
                "STRICT mode enforces that all values match their declared types. This table has data that cannot be converted. Consider using the Query Console to fix problematic data first.",
            }),
            {
              status: 400,
              headers: jsonHeaders(corsHeaders),
        ...
        await executeQueryViaAPI(
          dbId,
          `ALTER TABLE "${sanitizedTempTable}" RENAME TO "${sanitizedTable}"`,
          env,
        );
        ...
        const updatedTableListResult = await executeQueryViaAPI(
          dbId,
          "PRAGMA table_list",
          env,
        );
        const updatedTableInfo = (
          updatedTableListResult.results as TableInfo[]
        ).find((t: TableInfo) => t.name === tableName);
        ...
        return new Response(
          JSON.stringify({
            result: updatedTableInfo ?? {
              name: tableName,
              type: "table",
              ncol: normalColumns.length,
              wr: 0,
              strict: 1,
            },
            success: true,
    ...
    if (
      request.method === "GET" &&
      /^\/api\/tables\/[^/]+\/[^/]+\/export$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[4] ?? "");
      ...
      if (isLocalDev) {
        ...
        const fileExt =
          format === "json" ? "json" : format === "csv" ? "csv" : "sql";
        return new Response(
          JSON.stringify({
            result: {
              content: mockContent,
    ...
    if (
      request.method === "POST" &&
      /^\/api\/tables\/[^/]+\/[^/]+\/columns\/add$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[4] ?? "");
      logInfo(`Adding column to table: ${tableName}`, {
        module: "tables",
          ...
          JSON.stringify({
            result: [
              ...
              },
              {
                ...
                name: body.name,
                type: body.type,
                notnull: body.notnull ? 1 : 0,
                dflt_value: body.defaultValue ?? null,
                pk: 0,
              },
            ],
            success: true,
      ...
      const jobId = await startJobTracking(
        ...
        {
          tableName,
          columnName: body.name,
          columnType: body.type,
    ...
    if (
      request.method === "PATCH" &&
      /^\/api\/tables\/[^/]+\/[^/]+\/columns\/[^/]+\/rename$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[4] ?? "");
      const columnName = decodeURIComponent(pathParts[6] ?? "");
      logInfo(`Renaming column: ${columnName} in table: ${tableName}`, {
        module: "tables",
    ...
    if (
      request.method === "PATCH" &&
      /^\/api\/tables\/[^/]+\/[^/]+\/columns\/[^/]+\/modify$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[4] ?? "");
      const columnName = decodeURIComponent(pathParts[6] ?? "");
      logInfo(`Modifying column: ${columnName} in table: ${tableName}`, {
        module: "tables",
      ...
      try {
        ...
        await recreateTableWithModifiedColumn(
          dbId,
          tableName,
          modificationParams,
          env,
    ...
    if (
      request.method === "DELETE" &&
      /^\/api\/tables\/[^/]+\/[^/]+\/columns\/[^/]+$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[4] ?? "");
      const columnName = decodeURIComponent(pathParts[6] ?? "");
      logInfo(`Deleting column: ${columnName} from table: ${tableName}`, {
        module: "tables",
      ...
      try {
        ...
        try {
          const snapshot = await captureColumnSnapshot(
            dbId,
            tableName,
            columnName,
            env,
    ...
    if (
      request.method === "POST" &&
      /^\/api\/tables\/[^/]+\/[^/]+\/rows\/delete$/.exec(url.pathname)
    ) {
      const tableName = decodeURIComponent(pathParts[4] ?? "");
      logInfo(`Deleting rows from table: ${tableName}`, {
        module: "tables",
      ...
      try {
        ...
        try {
          const snapshot = await captureRowSnapshot(
            dbId,
            tableName,
            body.whereClause,
            env,
          ...
          await saveUndoSnapshot(
            ...
            null,
            body.description ??
              `Deleted ${String(rowCount)} row(s) from table "${tableName}"`,
            snapshot,
            userEmail,
            env,
          );
    ...
    if (
      request.method === "GET" &&
      url.pathname === `/api/tables/${dbId}/dependencies`
    ) {
      ...
      logInfo(`Getting dependencies for tables: ${tableNames.join(", ")}`, {
        module: "tables",
      ...
      for (const tableInfo of allTables) {
        ...
        try {
          ...
          logWarning(
            `Could not get FK info for table ${tableInfo.name}: ${err instanceof Error ? err.message : String(err)}`,
            {
              module: "tables",
              operation: "dependencies",
              databaseId: dbId,
      ...
      for (let i = 0; i < tablesToCount.length; i += BATCH_SIZE) {
        ...
        for (const tableName of batch) {
          ...
          try {
            ...
            const firstResult = countResult.results[0] as
              | { count: number }
              | undefined;
      ...
      logInfo(
        `Dependencies computed for ${String(tableNames.length)} tables using optimized FK index`,
        {
          module: "tables",
          operation: "dependencies",
          databaseId: dbId,
    ...
    if (
      request.method === "POST" &&
      url.pathname === `/api/tables/${dbId}/simulate-cascade`
    ) {
      logInfo("Simulating cascade impact", {
        module: "tables",
        operation: "cascade_impact",
        databaseId: dbId,
      });
          ...
          warnings: [
            {
              type: "high_impact",
              message:
                "Deletion will cascade to 286 additional rows across 4 tables",
              severity: "high",
            },
            {
              type: "deep_cascade",
      ...
      const simulation = await simulateCascadeImpact(
        dbId,
        body.targetTable,
        body.whereClause,
        env,
    ...
    if (
      request.method === "GET" &&
      url.pathname === `/api/tables/${dbId}/foreign-keys`
    ) {
      const includeCycles = url.searchParams.get("includeCycles") === "true";
      const includeSchemas = url.searchParams.get("includeSchemas") === "true";
      logInfo(
        `Getting all foreign keys for database: ${dbId}${includeCycles ? " (with cycles)" : ""}${includeSchemas ? " (with schemas)" : ""}`,
        {
          module: "tables",
          operation: "all_foreign_keys",
          databaseId: dbId,
          metadata: { includeCycles, includeSchemas },
        },
      );
          ...
          nodes: [
            {
              id: "users",
              label: "users",
              columns: [{ name: "id", type: "INTEGER", isPK: true }],
              rowCount: 50,
            },
            {
              id: "posts",
            ...
            },
            {
              ...
              columns: [
                { name: "id", type: "INTEGER", isPK: true },
                { name: "post_id", type: "INTEGER", isPK: false },
              ],
              rowCount: 340,
            },
          ],
          ...
          mockResult.cycles = [
            {
              ...
              message:
                "Circular dependency detected: users → profiles → users (contains RESTRICT constraints)",
            },
          ];
        }
      ...
      const fkGraphResult = await getAllForeignKeysForDatabase(
        dbId,
        env,
        includeSchemas,
      );
      ...
      if (includeCycles) {
        const { detectCircularDependencies } =
          await import("../utils/circular-dependency-detector");
    ...
    if (
      request.method === "GET" &&
      url.pathname === `/api/tables/${dbId}/circular-dependencies`
    ) {
      logInfo(`Detecting circular dependencies for database: ${dbId}`, {
        module: "tables",
      ...
      const { detectCircularDependencies } =
        await import("../utils/circular-dependency-detector");
      const fkGraphResult = await getForeignKeyGraphForCycleDetection(
        dbId,
        env,
      );
      const cycles = detectCircularDependencies(fkGraphResult);
    ...
    if (
      request.method === "POST" &&
      url.pathname === `/api/tables/${dbId}/foreign-keys/simulate`
    ) {
      logInfo("Simulating foreign key addition", {
        module: "tables",
        operation: "fk_simulate",
        databaseId: dbId,
      });
      ...
      const simulation = wouldCreateCycle(
        fkGraphResult,
        body.sourceTable,
        body.targetTable,
      );
    ...
    if (
      request.method === "POST" &&
      url.pathname === `/api/tables/${dbId}/foreign-keys/add`
    ) {
      logInfo("Adding foreign key constraint", {
        module: "tables",
        operation: "fk_add",
        databaseId: dbId,
      });
      ...
      if (
        ...
        return new Response(
          JSON.stringify({
            error:
              "sourceTable, sourceColumn, targetTable, and targetColumn are required",
          }),
          {
            status: 400,
            headers: jsonHeaders(corsHeaders),
      ...
      if (isLocalDev) {
        return new Response(
          JSON.stringify({
            result: {
              success: true,
              message: "Foreign key added successfully (mock)",
            },
            success: true,
          }),
          {
      ...
      try {
        ...
        void logError(
          env,
          `Foreign key add error: ${errorMessage}`,
          { module: "tables", operation: "fk_add", databaseId: dbId },
          isLocalDev,
        );
        return new Response(
          JSON.stringify({
    ...
    if (
      ...
      logInfo(`Modifying foreign key constraint: ${constraintName}`, {
        module: "tables",
        operation: "fk_modify",
        databaseId: dbId,
      ...
      if (isLocalDev) {
        return new Response(
          JSON.stringify({
            result: {
              success: true,
              message: "Foreign key modified successfully (mock)",
            },
            success: true,
          }),
          {
      ...
      try {
        ...
        void logError(
          env,
          `Foreign key modify error: ${errorMessage}`,
          {
            module: "tables",
            operation: "fk_modify",
            databaseId: dbId,
    ...
    if (
      ...
      const constraintName = decodeURIComponent(pathParts[5] ?? "");
      logInfo(`Deleting foreign key constraint: ${constraintName}`, {
        module: "tables",
        operation: "fk_delete",
        databaseId: dbId,
        metadata: { constraintName },
      ...
      if (isLocalDev) {
        return new Response(
          JSON.stringify({
            result: {
              success: true,
              message: "Foreign key deleted successfully (mock)",
            },
            success: true,
          }),
          {
      ...
      try {
        ...
        void logError(
          env,
          `Foreign key delete error: ${errorMessage}`,
          {
            module: "tables",
            operation: "fk_delete",
            databaseId: dbId,
    ...
    if (
      request.method === "GET" &&
      url.pathname === `/api/tables/${dbId}/schema-full`
    ) {
      logInfo(`Getting full schema for database: ${dbId}`, {
        module: "tables",
        operation: "schema_full",
        databaseId: dbId,
      });
      ...
      try {
          ...
          const batchResults = await Promise.all(
            batch.map(async (table) => {
              ...
              try {
                ...
                const [schemaResult, indexListResult, fkResult] =
                  await Promise.all([
                    executeQueryViaAPI(
                      dbId,
                      `PRAGMA table_info("${sanitizedTable}")`,
                ...
                const tableIndexes = indexListResult.results as {
                  seq: number;
                  name: string;
                  unique: number;
                  origin: string;
                ...
                for (const idx of tableIndexes) {
                  if (idx.origin !== "pk") {
                    ...
                    try {
                      ...
                      const idxCols = (
                        idxInfoResult.results as { name: string }[]
                      ).map((c) => c.name);
                      indexDetails.push({
                        table: table.name,
                        name: idx.name,
                ...
                logWarning(
                  `Skipping table "${table.name}" in schema-full: ${err instanceof Error ? err.message : "Unknown error"}`,
                  {
                    module: "tables",
        ...
        try {
          ...
          const triggerRows = triggerResult.results as {
            name: string;
            table_name: string;
            sql: string;
          }[];
        ...
        void logError(
          env,
          err instanceof Error ? err : String(err),
          { module: "tables", operation: "schema_full", databaseId: dbId },
          isLocalDev,
        );
        return new Response(
          JSON.stringify({
    ...
    void logError(
      env,
      err instanceof Error ? err : String(err),
      { module: "tables", operation: "request", databaseId: dbId },
      isLocalDev,
    );
    ...
    if (errorStr.includes("Currently processing a long-running export")) {
      errorMessage =
        "Database is currently processing an export. Please wait a few minutes and try again.";
  ...
  try {
    ...
    if (modification.action === "drop") {
      ...
      newColumns = columns.filter(
        (col) => col.name !== modification.columnName,
      );
    ...
    try {
      ...
      logWarning(
        `Failed to clean up temp table: ${cleanupErr instanceof Error ? cleanupErr.message : String(cleanupErr)}`,
        { module: "tables", operation: "cleanup_temp_table", databaseId: dbId },
      );
    }
    throw err;
  }
...
async function simulateCascadeImpact(
  ...
  const countResult = await executeQueryViaAPI(dbId, countQuery, env);
  const targetCountRow = countResult.results[0] as
    | { count: number }
    | undefined;
  ...
  const affectedTablesMap = new Map<
    string,
    {
      tableName: string;
      action: string;
    ...
    for (const edge of referencingEdges) {
      ...
      if (affectedRows > 0) {
        ...
        if (action.toUpperCase() === "CASCADE") {
          ...
          action.toUpperCase() === "SET NULL" ||
          action.toUpperCase() === "SET DEFAULT"
        ) {
          ...
          action.toUpperCase() === "RESTRICT" ||
          action.toUpperCase() === "NO ACTION"
        ) {
  ...
  if (totalAffected > targetRowCount) {
    ...
    warnings.push({
      ...
      severity:
        additionalRows > 100 ? "high" : additionalRows > 10 ? "medium" : "low",
    });
  }
      ...
      batch.map(async (table) => {
        ...
        try {
          ...
          const [schemaResult, countResult, fkResult] = await Promise.all([
            ...
            executeQueryViaAPI(
              dbId,
              `SELECT COUNT(*) as count FROM "${sanitizedTable}"`,
              env,
            ),
            executeQueryViaAPI(
              dbId,
          ...
          logWarning(
            `Skipping table "${table.name}" in FK graph: ${err instanceof Error ? err.message : "Unknown error"}`,
            {
              module: "tables",
...
async function getForeignKeyGraphForCycleDetection(
    ...
    const batchResults = await Promise.all(
      batch.map(async (table) => {
        ...
        try {
          ...
          logWarning(
            `Skipping table "${table.name}" in cycle detection: ${err instanceof Error ? err.message : "Unknown error"}`,
            {
              module: "tables",
  ...
  logInfo(
    `Built lightweight FK graph for cycle detection: ${String(nodes.length)} tables, ${String(edges.length)} edges`,
    {
      module: "tables",
...
async function addForeignKeyConstraint(
  ...
  const sourceSchema = await executeQueryViaAPI(
    dbId,
    `PRAGMA table_info("${sanitizeIdentifier(sourceTable)}")`,
    env,
  );
  const targetSchema = await executeQueryViaAPI(
    dbId,
    `PRAGMA table_info("${sanitizeIdentifier(targetTable)}")`,
    env,
  );
  ...
  const sourceCol = (sourceSchema.results as ColumnInfoResult[]).find(
    (c) => c.name === sourceColumn,
  );
  const targetCol = (targetSchema.results as ColumnInfoResult[]).find(
  ...
  if (targetCol.pk === 0) {
    ...
    if (!hasUniqueIndex) {
      throw new Error(
        `Target column ${targetColumn} must have a UNIQUE constraint or be a PRIMARY KEY`,
      );
    }
  }
  ...
  if (orphanCount > 0) {
    throw new Error(
      `Cannot add foreign key: ${String(orphanCount)} rows in ${sourceTable} reference non-existent rows in ${targetTable}`,
    );
  }
  ...
  await recreateTableWithForeignKey(
    dbId,
    sourceTable,
    {
      action: "add",
...
async function modifyForeignKeyConstraint(
  ...
  if (
    ...
    throw new Error(
      "Invalid constraint name format. Expected: fk__sourceTable__sourceColumn__targetTable__targetColumn",
    );
  }
  ...
  const currentFk = fks.find(
    (fk) =>
      fk.table === targetTable &&
      fk.from === sourceColumn &&
      fk.to === targetColumn,
  );
  if (!currentFk) {
  ...
  const onDelete =
    params.onDelete?.toUpperCase() ?? currentFk.on_delete ?? "NO ACTION";
  const onUpdate =
    params.onUpdate?.toUpperCase() ?? currentFk.on_update ?? "NO ACTION";
...
async function deleteForeignKeyConstraint(
  ...
  if (
    ...
    throw new Error(
      `Invalid constraint name format: ${constraintName}. Expected format: fk__table__column__refTable__refColumn`,
    );
  }

  logInfo(
    `Deleting FK constraint: ${sourceTable}.${sourceColumn} -> ${targetTable}.${targetColumn}`,
    {
      module: "tables",
  ...
  try {
    ...
    if (modification.action === "modify" || modification.action === "remove") {
      ...
      if (newCreateSql === originalSql) {
        ...
        throw new Error(
          `Could not find foreign key constraint ${colName} -> ${refTbl}(${refCol}) in table "${tableName}". The constraint may have been defined inline or with different quoting.`,
        );
      }
    ...
    if (modification.action === "add" || modification.action === "modify") {
      const { columns, refTable, refColumns, onDelete, onUpdate, name } =
      ...
      const sanitizedTableForConstraint = tableName.replace(
        /[^a-zA-Z0-9_]/g,
        "_",
      );
      const sanitizedColumnsForConstraint = columns
        .map((c) => c.replace(/[^a-zA-Z0-9_]/g, "_"))
        .join("_");
      const constraintName =
        name ??
        `fk_${sanitizedTableForConstraint}_${sanitizedColumnsForConstraint}`;
    ...
    const indexResult = await executeQueryViaAPI(dbId, indexQuery, env);
    const indexes = (indexResult.results as { sql: string }[]).map(
      (r) => r.sql,
    );
...
async function executeQueryViaAPI(
  ...
  logInfo(
    `D1 API request - query length: ${query.length}, body length: ${requestBody.length}`,
    {
      module: "tables",
      operation: "api_debug",
      databaseId,
