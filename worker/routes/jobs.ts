...
export async function handleJobRoutes(
  ...
  if (url.pathname === "/api/jobs" && request.method === "GET") {
    logInfo(`Getting job list for user: ${userEmail}`, {
      module: "jobs",
      operation: "list",
      userId: userEmail,
    });
    ...
    try {
      ...
      logWarning(
        `Error listing jobs: ${error instanceof Error ? error.message : String(error)}`,
        { module: "jobs", operation: "list", userId: userEmail },
      );
  ...
  if (jobMatch && request.method === "GET") {
    ...
    logInfo(`Getting status for job: ${requestedJobId}`, {
      module: "jobs",
      operation: "status",
    ...
    try {
      ...
      logWarning(
        `Error getting job: ${error instanceof Error ? error.message : String(error)}`,
        {
          module: "jobs",
          operation: "status",
          metadata: { jobId: requestedJobId },
  ...
  if (eventsMatch && request.method === "GET") {
    ...
    logInfo(`Getting events for job: ${requestedJobId}`, {
      module: "jobs",
      operation: "events",
      metadata: { jobId: requestedJobId },
    });
    ...
    try {
      ...
      logWarning(
        `Error getting job events: ${error instanceof Error ? error.message : String(error)}`,
        {
          module: "jobs",
          operation: "events",
          metadata: { jobId: requestedJobId },
