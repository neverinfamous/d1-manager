...
function isR2Configured(
  env: Env,
): env is Env & { BACKUP_BUCKET: R2Bucket; BACKUP_DO: DurableObjectNamespace } {
...
export async function handleR2BackupRoutes(
  ...
  if (listMatch !== null && request.method === "GET") {
    ...
    if (isLocalDev || !isR2Configured(env)) {
      ...
      return new Response(
        JSON.stringify({ success: true, result: mockBackups }),
        {
  ...
  const tableBackupMatch = /^\/api\/r2-backup\/([^/]+)\/table$/.exec(
    url.pathname,
  );
    ...
    if (!isR2Configured(env)) {
      return new Response(
        JSON.stringify({
          success: false,
          error:
            "R2 backup is not configured. Please add BACKUP_BUCKET and BACKUP_DO bindings to your wrangler.toml and redeploy.",
        }),
        {
          status: 503,
          headers: jsonHeaders(corsHeaders),
  ...
  if (backupMatch !== null && request.method === "POST") {
    ...
    if (!isR2Configured(env)) {
      ...
      logInfo(
        `R2 not configured - bucket: ${bucketAvailable}, DO: ${doAvailable}`,
        {
          module: "r2_backup",
          operation: "backup",
          databaseId,
        ...
        stub.fetch(doRequest).then(
          async (res) => {
            if (!res.ok) {
              ...
              void logError(
                env,
                `DO backup failed: ${res.status} - ${responseText}`,
                {
                  module: "r2_backup",
  ...
  const downloadMatch = /^\/api\/r2-backup\/([^/]+)\/download\/(\d+)$/.exec(
    url.pathname,
  );
  ...
  if (deleteMatch !== null && request.method === "DELETE") {
    ...
    try {
      ...
      const backupPath =
        explicitPath ?? `backups/${databaseId}/${timestamp}.sql`;
  ...
  const bulkDeleteMatch = /^\/api\/r2-backup\/([^/]+)\/bulk$/.exec(
    url.pathname,
  );
    ...
    try {
      ...
      for (const timestamp of timestamps) {
        try {
          ...
          failed++;
          errors.push(
            `Failed to delete backup ${String(timestamp)}: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      }

      logInfo(
        `Bulk delete completed: ${String(deleted)} deleted, ${String(failed)} failed`,
        {
          module: "r2_backup",
          operation: "bulk_delete",
          databaseId,
    ...
    try {
      ...
      for (const obj of listed.objects) {
        try {
          ...
          failed++;
          errors.push(
            `Failed to delete ${obj.key}: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      }

      logInfo(
        `Delete all completed: ${String(deleted)} deleted, ${String(failed)} failed`,
        {
          module: "r2_backup",
          operation: "delete_all",
          databaseId,
  ...
  if (url.pathname === "/api/r2-backup/orphaned" && request.method === "GET") {
    ...
    try {
      ...
      do {
        ...
        const dbListResponse = await fetch(url.toString(), {
          headers: cfHeaders,
        });

        if (!dbListResponse.ok) {
          logWarning(
            `Failed to fetch database list: ${dbListResponse.status}`,
            {
              module: "r2_backup",
              operation: "list_orphaned",
              userId: userEmail,
      ...
      if (orphanedDbIds.length > 0) {
        try {
          ...
          logInfo(
            `Database name lookup: found ${String(dbNameLookup.size)} of ${String(orphanedDbIds.length)} names`,
            {
              module: "r2_backup",
              operation: "list_orphaned",
              userId: userEmail,
          ...
          logWarning(
            `Database name lookup failed: ${lookupErr instanceof Error ? lookupErr.message : String(lookupErr)}`,
            {
              module: "r2_backup",
              operation: "list_orphaned",
              userId: userEmail,
      ...
      for (const [dbId, backups] of backupsByDb) {
        ...
        if (
          ...
          databaseName === "" ||
          databaseName === "Deleted Database"
        ) {
          // Try to find a valid name from backup metadata
          const backupWithName = backups.find(
            (b) =>
      ...
      orphanedGroups.sort(
        (a, b) =>
          (b.backups[0]?.timestamp ?? 0) - (a.backups[0]?.timestamp ?? 0),
      );
      ...
      return new Response(
        JSON.stringify({ success: true, result: orphanedGroups }),
        {
          headers: jsonHeaders(corsHeaders),
        },
      );
