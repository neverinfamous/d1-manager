...
export async function handleSavedQueriesRoutes(
  ...
  logInfo("Handling saved queries operation", {
  ...
  try {
    ...
    if (
      ...
      url.pathname.startsWith("/api/saved-queries/")
    ) {
    ...
    logWarning(`Error: ${err instanceof Error ? err.message : String(err)}`, {
      module: "saved_queries",
      operation: "request",
    });
    ...
    if (
      ...
      errorMessage.includes("saved_queries.name")
    ) {
      return new Response(
        JSON.stringify({
    ...
    return new Response(
      JSON.stringify({
        ...
        message:
          "Unable to complete saved queries operation. Please try again.",
      }),
      {
        status: 500,
        headers: jsonHeaders(corsHeaders),
