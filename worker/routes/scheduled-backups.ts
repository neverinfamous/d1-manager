...
async function listSchedules(
  ...
  try {
    ...
    return jsonResponse(
      { success: true, result: schedulesWithDesc },
      corsHeaders,
    );
  } catch (error) {
...
async function createOrUpdateSchedule(
  ...
  if (!database_id || !database_name || !schedule) {
    return errorResponse(
      "database_id, database_name, and schedule are required",
      corsHeaders,
      400,
    );
  }
  ...
  if (isLocalDev) {
    const existingIndex = MOCK_SCHEDULES.findIndex(
      (s) => s.database_id === database_id,
    );
    const newSchedule: ScheduledBackup = {
      id:
        existingIndex >= 0
          ? (MOCK_SCHEDULES[existingIndex]?.id ?? generateScheduleId())
          : generateScheduleId(),
      database_id,
      database_name,
      schedule: schedule as ScheduledBackupSchedule,
      ...
      last_run_at:
        existingIndex >= 0
          ? (MOCK_SCHEDULES[existingIndex]?.last_run_at ?? null)
          : null,
      next_run_at: nextRunAt,
      last_job_id:
        existingIndex >= 0
          ? (MOCK_SCHEDULES[existingIndex]?.last_job_id ?? null)
          : null,
      last_status:
        existingIndex >= 0
          ? (MOCK_SCHEDULES[existingIndex]?.last_status ?? null)
          : null,
      created_at:
        existingIndex >= 0
          ? (MOCK_SCHEDULES[existingIndex]?.created_at ?? nowISO())
          : nowISO(),
      updated_at: nowISO(),
...
async function toggleSchedule(
  ...
  try {
    ...
    `,
      ...
      .bind(newEnabled, now, databaseId)
      .run();
