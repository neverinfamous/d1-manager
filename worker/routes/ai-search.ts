...
async function exportDatabaseToR2(
    ...
    if (!env.BACKUP_BUCKET) {
        return new Response(JSON.stringify({
            ...
            message: 'BACKUP_BUCKET binding required for AI Search export. Add it to wrangler.toml.',
        }), {
            status: 400,
    ...
    if (isLocalDev) {
        ...
        return new Response(JSON.stringify({
            ...
            filesExported: ['schema.md', 'tables/users.md', 'relationships.md', 'data/users.md'],
        }), {
            headers: { 'Content-Type': 'application/json', ...corsHeaders },
        });
    }
    ...
    try {
        ...
        const tablesData = await tablesResponse.json() as CloudflareApiResponse<{ results: { name: string }[] }[]>;
        ...
        const tableSchemas: { name: string; createSql: string; columns: string[] }[] = [];
        ...
        for (const table of tables) {
            ...
            const schemaData = await schemaResponse.json() as CloudflareApiResponse<{ results: { sql: string }[] }[]>;
            ...
            const columnsData = await columnsResponse.json() as CloudflareApiResponse<{ results: { name: string; type: string; notnull: number; pk: number }[] }[]>;
            const columns = columnsData.result?.[0]?.results ?? [];

            tableSchemas.push({
                name: tableName,
                createSql,
                columns: columns.map(c => `${c.name} (${c.type}${c.pk ? ', PRIMARY KEY' : ''}${c.notnull ? ', NOT NULL' : ''})`),
            });
        ...
        for (const table of tables) {
            const fkResponse = await fetch(
                `${CF_API}/accounts/${env.ACCOUNT_ID}/d1/database/${databaseId}/query`,
                {
                    method: 'POST',
                    headers: cfHeaders,
                    body: JSON.stringify({ sql: `PRAGMA foreign_key_list("${table.name}")` }),
                }
            );

            const fkData = await fkResponse.json() as CloudflareApiResponse<{ results: { table: string; from: string; to: string }[] }[]>;
        ...
        await env.BACKUP_BUCKET.put(`${exportPath}/relationships.md`, relationshipsContent, {
            customMetadata: {
                databaseId,
                databaseName,
                type: 'relationships',
        ...
        for (const table of tables) {
            ...
            const countResponse = await fetch(
                `${CF_API}/accounts/${env.ACCOUNT_ID}/d1/database/${databaseId}/query`,
                {
                    method: 'POST',
                    headers: cfHeaders,
                    body: JSON.stringify({ sql: `SELECT COUNT(*) as count FROM "${tableName}"` }),
                }
            );

            const countData = await countResponse.json() as CloudflareApiResponse<{ results: { count: number }[] }[]>;
            ...
            while (offset < rowCount) {
                ...
                const dataResult = await dataResponse.json() as CloudflareApiResponse<{ results: Record<string, unknown>[] }[]>;
            ...
            await env.BACKUP_BUCKET.put(`${exportPath}/data/${tableName}.md`, dataContent, {
                customMetadata: {
                    databaseId,
                    databaseName,
                    tableName,
            ...
            if (tableSchema) {
                ...
                await env.BACKUP_BUCKET.put(`${exportPath}/tables/${tableName}.md`, tableDoc, {
                    customMetadata: {
                        databaseId,
                        databaseName,
                        tableName,
...
export async function handleAISearchRoutes(
    ...
    try {
        ...
        if (request.method === 'POST' && exportMatch?.[1] !== undefined) {
            ...
            return await exportDatabaseToR2(env, databaseId, databaseName, corsHeaders, isLocalDev);
        }
        ...
        if (request.method === 'GET' && compatibilityMatch?.[1] !== undefined) {
            ...
            logInfo('Checking AI Search compatibility', { module: 'ai_search', operation: 'compatibility', databaseId });
            ...
            try {
                ...
                let totalRows = 0;
                for (const table of tables.slice(0, 10)) { // Limit to first 10 for performance
                    const countResponse = await fetch(
                        `${CF_API}/accounts/${env.ACCOUNT_ID}/d1/database/${databaseId}/query`,
                        {
                            method: 'POST',
                            headers: cfHeaders,
                            body: JSON.stringify({ sql: `SELECT COUNT(*) as count FROM "${table.name}"` }),
                        }
                    );
                    const countData = await countResponse.json() as CloudflareApiResponse<{ results: { count: number }[] }[]>;
                ...
                if (env.BACKUP_BUCKET) {
                    const schemaFile = await env.BACKUP_BUCKET.head(`ai-search/${databaseId}/schema.md`);
                    if (schemaFile) {
                ...
                const response: AISearchCompatibility = {
                    ...
                    ...(lastExport !== undefined && { exportPath: `ai-search/${databaseId}/` }),
                };
                ...
                return new Response(JSON.stringify({ error: 'Failed to analyze database' }), {
                    status: 500,
                    headers: { 'Content-Type': 'application/json', ...corsHeaders },
                });
            }
        ...
        if (request.method === 'GET' && url.pathname === '/api/ai-search/instances') {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    return new Response(JSON.stringify({
                        instances: [],
                        error: 'AI Search API not available. Create instances via Cloudflare Dashboard.',
                ...
                const data = JSON.parse(responseText) as CloudflareApiResponse<AISearchInstance[]>;
        ...
        if (request.method === 'POST' && syncMatch?.[1] !== undefined) {
            ...
            logInfo('Triggering AI Search sync', { module: 'ai_search', operation: 'sync', metadata: { instanceName } });
        ...
        if (request.method === 'POST' && searchMatch?.[1] !== undefined) {
            ...
            logInfo('Semantic search query', { module: 'ai_search', operation: 'search', metadata: { instanceName } });
                    ...
                    data: [
                        {
                            ...
                            filename: 'schema.md',
                            score: 0.92,
                            content: [{ id: '1', type: 'text', text: 'CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT, email TEXT UNIQUE)' }],
                        },
                        {
                            file_id: 'mock-2',
                            filename: 'tables/users.md',
                            score: 0.85,
                            content: [{ id: '2', type: 'text', text: 'The users table contains user account information with email validation.' }],
                        },
                    ],
                    has_more: false,
                    next_page: null,
        ...
        if (request.method === 'POST' && aiSearchMatch?.[1] !== undefined) {
            ...
            const body = await request.json() as AISearchQueryRequest;
            logInfo('AI Search query', { module: 'ai_search', operation: 'ai_search', metadata: { instanceName } });

            if (isLocalDev) {
                const mockResponse: AISearchResponse = {
                    response: 'Based on your database schema, the `users` table has columns for id, name, and email. You can query users by email using: `SELECT * FROM users WHERE email = ?`',
                    data: [
                        {
                            file_id: 'mock-1',
                            filename: 'schema.md',
