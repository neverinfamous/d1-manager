...
import {
  triggerWebhooks,
  createImportCompletePayload,
  createExportCompletePayload,
} from "../utils/webhooks";
...
async function hasFTS5Tables(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      void logError(
        env,
        `Failed to query for FTS5 tables: ${errorText}`,
        {
          module: "databases",
          operation: "fts5_check",
          databaseId,
...
async function verifyDatabaseIntegrity(
  ...
  try {
    ...
    const sourceQueryResults = sourceTablesData.result as
      | D1QueryResult[]
      | undefined;
    ...
    const targetQueryResults = targetTablesData.result as
      | D1QueryResult[]
      | undefined;
    ...
    if (sourceTables.length !== targetTables.length) {
      ...
      issues.push(
        `Table count mismatch: source has ${String(sourceTables.length)}, target has ${String(targetTables.length)}`,
      );
    }
    ...
    for (const tableName of sourceTables) {
      ...
      try {
        ...
        if (sourceCountResponse.ok && targetCountResponse.ok) {
          const sourceCountData: D1APIResponse =
            await sourceCountResponse.json();
          const targetCountData: D1APIResponse =
            await targetCountResponse.json();

          const sourceCountResults = sourceCountData.result as
            | D1QueryResult[]
            | undefined;
          ...
          if (!sourceCountData.success || !srcRow) {
            logWarning(
              `Invalid source count response for table "${tableName}"`,
              {
                module: "databases",
                operation: "verify",
                databaseId: sourceDbId,
            ...
            issues.push(
              `Failed to get row count for source table "${tableName}"`,
            );
            continue;
          }

          const targetCountResults = targetCountData.result as
            | D1QueryResult[]
            | undefined;
          ...
          if (!targetCountData.success || !tgtRow) {
            logWarning(
              `Invalid target count response for table "${tableName}"`,
              {
                module: "databases",
                operation: "verify",
                databaseId: targetDbId,
            ...
            issues.push(
              `Failed to get row count for target table "${tableName}"`,
            );
            continue;
          }
          ...
          logInfo(
            `Table "${tableName}": source=${String(sourceCount)}, target=${String(targetCount)}`,
            {
              module: "databases",
              operation: "verify",
          ...
          if (sourceCount !== targetCount) {
            ...
            issues.push(
              `Row count mismatch in table "${tableName}": source has ${String(sourceCount)}, target has ${String(targetCount)}`,
            );
          }
        } else {
        ...
        logWarning(
          `Error checking row count for table "${tableName}": ${err instanceof Error ? err.message : "Unknown error"}`,
          {
            module: "databases",
            operation: "verify",
            metadata: { tableName },
        ...
        issues.push(
          `Error verifying table "${tableName}": ${err instanceof Error ? err.message : "Unknown error"}`,
        );
      }
    }
    ...
    for (const tableName of sourceTables) {
      ...
      try {
        ...
        if (sourceSchemaResponse.ok && targetSchemaResponse.ok) {
          const sourceSchemaData: D1APIResponse =
            await sourceSchemaResponse.json();
          const targetSchemaData: D1APIResponse =
            await targetSchemaResponse.json();

          const sourceSchemaResults = sourceSchemaData.result as
            | D1QueryResult[]
            | undefined;
          const srcSchemaResult = sourceSchemaResults?.[0];
          if (!sourceSchemaData.success || !srcSchemaResult?.results) {
            logWarning(
              `Invalid source schema response for table "${tableName}"`,
              {
                module: "databases",
                operation: "verify",
                databaseId: sourceDbId,
          ...
          const targetSchemaResults = targetSchemaData.result as
            | D1QueryResult[]
            | undefined;
          const tgtSchemaResult = targetSchemaResults?.[0];
          if (!targetSchemaData.success || !tgtSchemaResult?.results) {
            logWarning(
              `Invalid target schema response for table "${tableName}"`,
              {
                module: "databases",
                operation: "verify",
                databaseId: targetDbId,
          ...
          logInfo(
            `Table "${tableName}": source has ${String(sourceColumns.length)} columns, target has ${String(targetColumns.length)} columns`,
            {
              module: "databases",
              operation: "verify",
              metadata: {
          ...
          if (sourceColumns.length !== targetColumns.length) {
            ...
            issues.push(
              `Column count mismatch in table "${tableName}": source has ${String(sourceColumns.length)}, target has ${String(targetColumns.length)}`,
            );
          }
        } else {
        ...
        logWarning(
          `Error checking schema for table "${tableName}": ${err instanceof Error ? err.message : "Unknown error"}`,
          {
            module: "databases",
            operation: "verify",
            metadata: { tableName },
        ...
        issues.push(
          `Error verifying schema for table "${tableName}": ${err instanceof Error ? err.message : "Unknown error"}`,
        );
      }
    }
    ...
    logWarning(
      `Fatal verification error: ${err instanceof Error ? err.message : "Unknown error"}`,
      {
        module: "databases",
        operation: "verify",
      },
...
export async function handleDatabaseRoutes(
  ...
  try {
    ...
    if (request.method === "GET" && url.pathname === "/api/databases") {
      ...
      logInfo(
        `Making API request to: ${CF_API}/accounts/${env.ACCOUNT_ID}/d1/database`,
        {
          module: "databases",
          operation: "list",
        },
      ...
      const filteredDatabases = databases.filter(
        (db: D1DatabaseInfo) => !isProtectedDatabase(db.name),
      );
        ...
        filteredDatabases.map(async (db: D1DatabaseInfo) => {
          try {
            ...
            if (statsResponse.ok) {
              ...
              const statsResults = statsData.result as
                | D1QueryResult[]
                | undefined;
    ...
    if (
      request.method === "GET" &&
      /^\/api\/databases\/[^/]+\/info$/.exec(url.pathname)
    ) {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to get database info: ${String(response.status)}`,
        );
      }
      ...
      if (isProtectedDatabase(dbInfo.name)) {
        logWarning(
          `Attempted to access protected database info: ${dbInfo.name}`,
          {
            module: "databases",
            operation: "get_info",
            databaseId: dbId,
    ...
    if (
      request.method === "PUT" &&
      /^\/api\/databases\/[^/]+\/replication$/.exec(url.pathname)
    ) {
      const dbId = url.pathname.split("/")[3] ?? "";
      const body: { mode?: string } = await request.json();
      logInfo(
        `Setting read replication for: ${dbId}, mode: ${body.mode ?? "undefined"}`,
        {
          module: "databases",
      ...
      if (!response.ok) {
        ...
        void logError(
          env,
          `Read replication update error: ${errorText}`,
          {
            module: "databases",
            operation: "set_replication",
            databaseId: dbId,
    ...
    if (request.method === "POST" && url.pathname === "/api/databases") {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Failed to create database: ${String(response.status)}`,
          );
        }
    ...
    if (
      ...
      /^\/api\/databases\/[^/]+$/.exec(url.pathname)
    ) {
      const dbId = url.pathname.split("/")[3] ?? "";
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Failed to delete database: ${String(response.status)}`,
          );
        }
    ...
    if (request.method === "POST" && url.pathname === "/api/databases/export") {
      ...
      if (isLocalDev) {
        ...
        for (const exportDbId of body.databaseIds) {
          mockExports[exportDbId] =
            `-- Mock export for database ${exportDbId}\nCREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, name TEXT);\nINSERT INTO users (id, name) VALUES (1, 'Test User');`;
        }
      ...
      for (const exportDbId of body.databaseIds) {
        try {
          ...
          if (dbInfoResponse.ok) {
            ...
            if (isProtectedDatabase(dbInfo.name)) {
              logWarning(
                `Skipping export of protected database: ${exportDbId} (${dbInfo.name})`,
                {
                  module: "databases",
                  operation: "export",
                  databaseId: exportDbId,
            ...
            void logError(
              env,
              `Failed to get database info for ${exportDbId}: ${errorText}`,
              {
                module: "databases",
                operation: "export",
                databaseId: exportDbId,
          ...
          const fts5Check = await hasFTS5Tables(
            exportDbId,
            cfHeaders,
            env,
            isLocalDev,
          ...
          if (fts5Check.hasFTS5) {
            void logError(
              env,
              `Cannot export database ${dbName} (${exportDbId}): contains FTS5 tables: ${fts5Check.fts5Tables.join(", ")}`,
              {
                module: "databases",
                operation: "export",
                databaseId: exportDbId,
          ...
          if (!startResponse.ok) {
            ...
            void logError(
              env,
              `Export start failed for ${dbName} (${exportDbId}): ${String(startResponse.status)} - ${errorText}`,
              {
                module: "databases",
                operation: "export",
                databaseId: exportDbId,
          ...
          if (
            startResult?.status === "complete" &&
            startResult.result?.signed_url
          ) {
            ...
            logInfo(
              `Got bookmark for ${dbName}: ${bookmark}, polling for completion...`,
              {
                module: "databases",
                operation: "export",
                databaseId: exportDbId,
            ...
            while (!signedUrl && attempts < maxAttempts) {
              ...
              if (pollResponse.ok) {
                ...
                const pollUrl =
                  ...
                  pollData.result?.result?.signed_url;
                if (pollUrl) {
                  logInfo(
                    `Export ready for ${dbName} after ${String(attempts + 1)} polls`,
                    {
                      module: "databases",
                      operation: "export",
                      databaseId: exportDbId,
                  ...
                  void logError(
                    env,
                    `Export poll error for ${dbName}: ${pollData.result.error}`,
                    {
                      module: "databases",
                      operation: "export",
                      databaseId: exportDbId,
                  ...
                  logInfo(
                    `Still waiting for ${dbName}... (attempt ${String(attempts + 1)}/${String(maxAttempts)})`,
                    {
                      module: "databases",
                      operation: "export",
                      databaseId: exportDbId,
                ...
                void logError(
                  env,
                  `Poll request failed for ${dbName}: ${String(pollResponse.status)} - ${errorText}`,
                  {
                    module: "databases",
                    operation: "export",
                    databaseId: exportDbId,
            ...
            if (!signedUrl) {
              void logError(
                env,
                `Export timeout for ${dbName} (${exportDbId}) after ${String(attempts)} attempts`,
                {
                  module: "databases",
                  operation: "export",
                  databaseId: exportDbId,
            ...
            void logError(
              env,
              `Export API did not return expected response for ${dbName}`,
              {
                module: "databases",
                operation: "export",
                databaseId: exportDbId,
          ...
          if (downloadResponse.ok) {
            ...
            logInfo(
              `Successfully exported ${dbName}: ${String(sqlContent.length)} bytes`,
              {
                module: "databases",
                operation: "export",
                databaseId: exportDbId,
            ...
            void logError(
              env,
              `Failed to download export for ${dbName}: ${String(downloadResponse.status)}`,
              {
                module: "databases",
                operation: "export",
                databaseId: exportDbId,
      ...
      if (skipped.length > 0) {
        ...
        const protectedSkipped = skipped.filter(
          (s) => s.reason === "protected",
        );
        const messages: string[] = [];

        if (fts5Skipped.length > 0) {
          messages.push(
            `FTS5 tables not supported: ${fts5Skipped.map((s) => s.name).join(", ")}`,
          );
        }
        if (protectedSkipped.length > 0) {
          messages.push(
            `Protected databases skipped: ${protectedSkipped.map((s) => s.name).join(", ")}`,
          );
        }
      ...
      try {
        const jobParams: {
          ...
          jobId,
          status:
            errorCount > 0 && Object.keys(exports).length === 0
              ? "failed"
              : "completed",
    ...
    if (request.method === "POST" && url.pathname === "/api/databases/import") {
      ...
      logInfo("Importing database", {
        module: "databases",
        operation: "import",
        ...(body.targetDatabaseId !== undefined && {
          databaseId: body.targetDatabaseId,
        }),
        ...(body.databaseName !== undefined && {
      ...
      try {
        ...
        if (body.createNew && body.databaseName) {
          ...
          if (!createResponse.ok) {
            ...
            void logError(
              env,
              `Create error during import: ${errorText}`,
              {
                module: "databases",
                operation: "import",
                databaseName: body.databaseName,
        ...
        if (!initResponse.ok) {
          ...
          throw new Error(
            `Failed to initialize import: ${String(initResponse.status)}`,
          );
        }
        ...
        logInfo(
          `Import Step 2: Uploading SQL to R2, filename: ${initData.result.filename ?? "unknown"}`,
          {
            module: "databases",
            operation: "import",
            databaseId: targetDbId,
        ...
        if (!uploadResponse.ok) {
          ...
          throw new Error(
            `Failed to upload SQL content: ${String(uploadResponse.status)}`,
          );
        }
        ...
        if (!ingestResponse.ok) {
          ...
          throw new Error(
            `Failed to start ingestion: ${String(ingestResponse.status)}`,
          );
        }
        ...
        logInfo(
          `Import Step 4: Polling for completion, bookmark: ${ingestData.result?.at_bookmark ?? "none"}`,
          {
            module: "databases",
            operation: "import",
            databaseId: targetDbId,
        ...
        if (ingestData.result?.at_bookmark) {
          ...
          while (pollAttempts < maxPollAttempts) {
            ...
            if (
              ...
              pollData.result.error !== "Not currently importing anything."
            ) {
        ...
        try {
          ...
          void logError(
            env,
            jobErr instanceof Error ? jobErr : String(jobErr),
            {
              module: "databases",
              operation: "import",
              metadata: { jobId },
    ...
    if (
      request.method === "POST" &&
      /^\/api\/databases\/[^/]+\/rename$/.exec(url.pathname)
    ) {
      const dbId = url.pathname.split("/")[3] ?? "";
      ...
      if (fts5Check.hasFTS5) {
        logWarning(
          `Cannot rename database with FTS5 tables: ${fts5Check.fts5Tables.join(", ")}`,
          {
            module: "databases",
            operation: "rename",
            databaseId: dbId,
      ...
      try {
        ...
        logInfo("Step 1: Validating new name", {
          module: "databases",
          operation: "rename",
          databaseId: dbId,
        });
        ...
        const existingDb = databases.find(
          (d: D1DatabaseInfo) => d.name === body.newName,
        );

        if (existingDb) {
          throw new Error(
            `Database with name "${body.newName ?? ""}" already exists`,
          );
        }

        // Step 2: Create new database with desired name
        logInfo("Step 2: Creating new database", {
          module: "databases",
          operation: "rename",
          databaseId: dbId,
          metadata: { newName: body.newName },
        ...
        if (!createResponse.ok) {
          ...
          void logError(
            env,
            `Create error during rename: ${errorText}`,
            {
              module: "databases",
              operation: "rename",
              databaseId: dbId,
          ...
          throw new Error(
            `Failed to create new database: ${String(createResponse.status)}`,
          );
        }
        ...
        const newDb = createData.result as D1DatabaseInfo;
        newDbId = newDb.uuid;
        logInfo(`Created new database: ${newDbId}`, {
          module: "databases",
          operation: "rename",
          databaseId: newDbId,
        });
        ...
        logInfo("Step 3: Exporting source database", {
          module: "databases",
          operation: "rename",
          databaseId: dbId,
        });
        ...
        const exportStartData: ExportAPIResponse =
          await startExportResponse.json();
        logInfo("Export API response received", {
          module: "databases",
          operation: "rename",
          databaseId: dbId,
        ...
        if (
          ...
          exportResult.result?.signed_url
        ) {
          logInfo("Export already complete (immediate)", {
            module: "databases",
            operation: "rename",
            databaseId: dbId,
          });
          ...
          logInfo("Export already complete (direct signed_url)", {
            module: "databases",
            operation: "rename",
            databaseId: dbId,
          });
          ...
          logInfo(`Got bookmark, polling for completion: ${bookmark}`, {
            module: "databases",
            operation: "rename",
            databaseId: dbId,
            metadata: { bookmark },
          ...
          while (!signedUrl && attempts < maxAttempts) {
            ...
            if (pollResponse.ok) {
              ...
              const pollUrl =
                ...
                pollData.result?.result?.signed_url;
              if (pollUrl) {
                logInfo(`Export ready after ${String(attempts + 1)} polls`, {
                  module: "databases",
                  operation: "rename",
                  databaseId: dbId,
                  metadata: { attempts: attempts + 1 },
                ...
                logInfo(
                  `Still waiting for export... (attempt ${String(attempts + 1)}/${String(maxAttempts)})`,
                  {
                    module: "databases",
                    operation: "rename",
                    databaseId: dbId,
              ...
              void logError(
                env,
                `Poll request failed: ${String(pollResponse.status)} ${errorText}`,
                {
                  module: "databases",
                  operation: "rename",
                  databaseId: dbId,
          ...
          void logError(
            env,
            "Export API did not return expected response",
            {
              module: "databases",
              operation: "rename",
              databaseId: dbId,
          ...
          throw new Error(
            "Failed to start database export - unexpected API response",
          );
        }
        ...
        logInfo("Downloading exported SQL", {
          module: "databases",
          operation: "rename",
          databaseId: dbId,
        });
        ...
        const createTableCount = (sqlContent.match(/CREATE TABLE/g) ?? [])
          .length;
        const hasTransaction =
          sqlContent.includes("BEGIN TRANSACTION") ||
          sqlContent.includes("BEGIN;") ||
          sqlContent.includes("COMMIT;");
        const hasStrict = sqlContent.includes("STRICT");

        logInfo(
          `Downloaded SQL content: ${String(sqlContent.length)} bytes, CREATE TABLE count: ${String(createTableCount)}, hasTransaction: ${String(hasTransaction)}, hasSTRICT: ${String(hasStrict)}`,
          {
            module: "databases",
            operation: "rename",
            databaseId: dbId,
        ...
        logInfo(
          `SQL preview: ${sqlPreview.replace(/\n/g, " ").substring(0, 200)}...`,
          {
            module: "databases",
            operation: "rename",
            databaseId: dbId,
        ...
        if (sqlContent.length !== originalLength) {
          logInfo(
            `SQL cleaned: removed problematic statements (${String(originalLength)} -> ${String(sqlContent.length)} bytes)`,
            {
              module: "databases",
              operation: "rename",
              databaseId: dbId,
        ...
        logInfo("Step 4: Importing into new database", {
          module: "databases",
          operation: "rename",
          databaseId: newDbId,
        });
        ...
        const etag = hashArray
          .map((b) => b.toString(16).padStart(2, "0"))
          .join("");
        logInfo(`Calculated etag (MD5): ${etag}`, {
          module: "databases",
          operation: "rename",
          databaseId: newDbId,
          metadata: { etag },
        ...
        logInfo("Step 4b: Initializing import upload", {
          module: "databases",
          operation: "rename",
          databaseId: newDbId,
        });
        ...
        const initData: ImportAPIResponse = await initResponse.json();
        logInfo(
          `Got upload URL and filename: ${initData.result?.filename ?? "unknown"}`,
          {
            module: "databases",
            operation: "rename",
            databaseId: newDbId,
        ...
        logInfo("Step 4c: Uploading SQL to R2", {
          module: "databases",
          operation: "rename",
          databaseId: newDbId,
        });
        ...
        logInfo(`R2 upload complete, ETag: ${r2Etag ?? "none"}`, {
          module: "databases",
          operation: "rename",
          databaseId: newDbId,
          metadata: { r2Etag },
        ...
        logInfo(
          `Step 4d: Starting ingestion with etag=${etag}, filename=${filename}, sqlSize=${String(sqlContent.length)}`,
          {
            module: "databases",
            operation: "rename",
            databaseId: newDbId,
        ...
        if (ingestData.result?.at_bookmark) {
          logInfo("Step 4e: Polling for import completion", {
            module: "databases",
            operation: "rename",
            databaseId: newDbId,
          });
            ...
            if (pollResponse.ok) {
              ...
              logInfo("Import poll response", {
                ...
                metadata: {
                  ...
                  resultKeys: pollData.result
                    ? Object.keys(pollData.result)
                    : [],
                },
              });
              ...
              if (pollData.result?.success) {
                logInfo(
                  `Import completed successfully after ${String(importAttempts + 1)} polls`,
                  {
                    module: "databases",
                    operation: "rename",
                    databaseId: newDbId,
                    metadata: { attempts: importAttempts + 1 },
                  },
                );
                importComplete = true;
              } else if (
                ...
                logWarning(
                  'Got "Not currently importing anything" - this may indicate the import was rejected. Checking table count...',
                  {
                    module: "databases",
                    operation: "rename",
                    databaseId: newDbId,
                ...
                try {
                  ...
                  if (checkResponse.ok) {
                    ...
                    const tableCount =
                      checkData.result?.[0]?.results?.length ?? 0;
                    logInfo(
                      `Quick table count check: ${String(tableCount)} tables found`,
                      {
                        module: "databases",
                        operation: "rename",
                        databaseId: newDbId,
                        metadata: {
                          tableCount,
                          tables: checkData.result?.[0]?.results?.map(
                            (r) => r.name,
                          ),
                        },
                      },
                  ...
                  logWarning("Failed to do quick table count check", {
                    module: "databases",
                    operation: "rename",
                    databaseId: newDbId,
                  });
                ...
                void logError(
                  env,
                  `Import poll error: ${pollData.result.error}`,
                  {
                    module: "databases",
                    operation: "rename",
                    databaseId: newDbId,
                ...
                logInfo(
                  `Still importing... (attempt ${String(importAttempts + 1)}/${String(maxImportAttempts)})`,
                  {
                    module: "databases",
                    operation: "rename",
                    databaseId: newDbId,
                    metadata: {
                      attempts: importAttempts + 1,
                      maxAttempts: maxImportAttempts,
                    },
                  },
                );
              }
              ...
              void logError(
                env,
                `Import poll request failed: ${String(pollResponse.status)} ${errorText}`,
                {
                  module: "databases",
                  operation: "rename",
                  databaseId: newDbId,
          ...
          logWarning(
            "No at_bookmark in ingest response - import may have failed silently",
            {
              module: "databases",
              operation: "rename",
              databaseId: newDbId,
        ...
        logInfo("Step 5: Verifying import integrity", {
          module: "databases",
          operation: "rename",
          databaseId: newDbId,
        });
        ...
        logInfo("Waiting 3 seconds for D1 eventual consistency...", {
          module: "databases",
          operation: "rename",
          databaseId: newDbId,
        });
        ...
        let verification: { success: boolean; issues: string[] } = {
          success: false,
          issues: [],
        };
        ...
        for (
          ...
          if (verification.success) {
            if (verifyAttempt > 0) {
              logInfo(
                `Verification passed on attempt ${String(verifyAttempt + 1)}`,
                {
                  module: "databases",
                  operation: "rename",
                  databaseId: newDbId,
          ...
          if (verifyAttempt < maxVerifyAttempts - 1) {
            logWarning(
              `Verification attempt ${String(verifyAttempt + 1)} failed, retrying in 3 seconds... Issues: ${verification.issues.join("; ")}`,
              {
                module: "databases",
                operation: "rename",
                databaseId: newDbId,
        ...
        if (!verification.success) {
          void logError(
            env,
            `Verification failed after ${String(maxVerifyAttempts)} attempts: ${verification.issues.join("; ")}`,
            {
              module: "databases",
              operation: "rename",
              databaseId: newDbId,
        ...
        logInfo("Verification passed - all data migrated successfully", {
          module: "databases",
          operation: "rename",
          databaseId: newDbId,
        });
        ...
        logInfo("Step 6: Deleting original database", {
          module: "databases",
          operation: "rename",
          databaseId: dbId,
        });
        ...
        if (!deleteResponse.ok) {
          logWarning(
            "Failed to delete original database - manual cleanup may be required",
            {
              module: "databases",
              operation: "rename",
              databaseId: dbId,
        ...
        logInfo("Rename completed successfully", {
          module: "databases",
          operation: "rename",
          databaseId: dbId,
          metadata: { newDbId, newName: body.newName },
        });
        ...
        if (newDbId) {
          logInfo(`Rolling back - deleting new database: ${newDbId}`, {
            module: "databases",
            operation: "rename",
            databaseId: newDbId,
          });
    ...
    if (
      request.method === "POST" &&
      url.pathname === "/api/databases/optimize"
    ) {
      const body: { databaseIds: string[] } = await request.json();
      ...
      // Mock response for local development
      if (isLocalDev) {
        logInfo("Simulating database optimization for local development", {
          module: "databases",
          operation: "optimize",
        });
        return new Response(
          JSON.stringify({
            result: {
              succeeded: body.databaseIds.map((id) => ({
                id,
                name: `mock-db-${id}`,
              })),
              failed: [],
      ...
      try {
        for (const optimizeDbId of body.databaseIds) {
          try {
            ...
            if (!queryResponse.ok) {
              ...
              void logError(
                env,
                `Optimize failed for ${optimizeDbId}: ${errorText}`,
                {
                  module: "databases",
                  operation: "optimize",
                  databaseId: optimizeDbId,
              ...
              failed.push({
                id: optimizeDbId,
                name: dbName,
                error: `Failed: ${String(queryResponse.status)}`,
              });
            } else {
        ...
        // Complete job tracking
        logInfo(
          `Optimize complete - Succeeded: ${String(succeeded.length)}, Failed: ${String(failed.length)}`,
          {
            module: "databases",
            operation: "optimize",
        ...
        const finalStatus =
          failed.length === body.databaseIds.length ? "failed" : "completed";
        await finishJobTracking(
          env,
          jobId,
          finalStatus,
        ...
        await finishJobTracking(env, jobId, "failed", userEmail, isLocalDev, {
          ...
          errorCount:
            failed.length +
            (body.databaseIds.length - succeeded.length - failed.length),
          errorMessage: err instanceof Error ? err.message : "Unknown error",
        });
        throw err;
      }
