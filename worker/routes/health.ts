 ...
 */
async function fetchAllDatabases(env: Env): Promise<D1DatabaseInfo[]> {
  const cfHeaders = {
  ...
  try {
    ...
    logWarning(
      `Error fetching databases: ${err instanceof Error ? err.message : String(err)}`,
      {
        module: "health",
        operation: "fetch_databases",
...
async function getHealthSummary(
  ...
  try {
    ...
    const [databases, scheduledBackupsResult, jobsResult, orphanedCountResult] =
      await Promise.all([
    ...
    const avgPerDatabase =
      dbCount > 0 ? Math.round(totalStorageBytes / dbCount) : 0;
    ...
    const dbsWithScheduledBackup = new Set(
      scheduledBackups.map((b) => b.database_id),
    );

    // Calculate orphaned backups count
    let orphanedCount = 0;
    if (
      "dbIds" in orphanedCountResult &&
      orphanedCountResult.dbIds instanceof Set
    ) {
    ...
    logWarning(
      `Failed to get health summary: ${err instanceof Error ? err.message : String(err)}`,
      {
        module: "health",
        operation: "get_summary",
        metadata: { error: err instanceof Error ? err.message : String(err) },
