...
function mapSqliteTypeToDrizzle(sqliteType: string): string {
  ...
  // Integer types
  if (upperType === 'INTEGER' || upperType === 'INT' || upperType === 'BIGINT' || upperType === 'SMALLINT' || upperType === 'TINYINT') {
    return 'integer';
  }
  
  // Real/float types
  if (upperType === 'REAL' || upperType === 'DOUBLE' || upperType === 'FLOAT' || upperType.startsWith('DOUBLE')) {
    return 'real';
  }
  
  // Text types
  if (upperType === 'TEXT' || upperType.startsWith('VARCHAR') || upperType.startsWith('CHAR') || 
      upperType === 'CLOB' || upperType.startsWith('NVARCHAR') || upperType.startsWith('NCHAR')) {
    return 'text';
  }
  
  // Blob type
  if (upperType === 'BLOB' || upperType === 'BINARY' || upperType.startsWith('VARBINARY')) {
    return 'blob';
  }
  ...
  if (upperType === 'DATE' || upperType === 'DATETIME' || upperType === 'TIMESTAMP') {
    return 'text';
  }
...
function generateColumnCode(column: DrizzleColumn, _tableName: string): string {
  ...
  if (column.defaultValue !== null) {
    ...
    if (defaultVal.toUpperCase() === 'CURRENT_TIMESTAMP' || defaultVal.toUpperCase() === 'CURRENT_DATE') {
  ...
  if (modifiers.length > 0) {
    ...
    if (column.isPrimaryKey && column.isAutoIncrement) {
      ...
      const otherModifiers = modifiers.filter(m => !m.startsWith('primaryKey') && !m.includes('autoIncrement'));
...
function generateDrizzleSchema(tables: DrizzleTable[]): string {
  ...
  const needsSql = tables.some(t => 
    t.columns.some(c => 
      ...
      (c.defaultValue.toUpperCase().includes('CURRENT_') || 
       (!c.defaultValue.startsWith("'") && isNaN(Number(c.defaultValue)) && c.defaultValue.toUpperCase() !== 'NULL'))
    )
  );
  ...
  for (const table of tables) {
    ...
    const camelName = tableName.replace(/_([a-z])/g, (_, letter: string) => letter.toUpperCase());
    
    code += `export const ${camelName} = sqliteTable('${tableName}', {\n`;
    
    // Generate columns
    const columnDefs = table.columns.map(col => generateColumnCode(col, tableName));
...
export async function introspectDatabase(
  ...
  try {
    ...
    const tableNames = (tablesResult.results as { name: string }[]).map(r => r.name);
    ...
    for (const tableName of tableNames) {
      ...
      if (tableName.endsWith('_content') || tableName.endsWith('_segments') || 
          tableName.endsWith('_segdir') || tableName.endsWith('_docsize') ||
          tableName.endsWith('_data') || tableName.endsWith('_idx') ||
      ...
      const columns: DrizzleColumn[] = (columnsResult.results as PragmaColumnInfo[]).map(col => ({
        name: col.name,
        type: col.type || 'TEXT',
      ...
      const foreignKeys = (fkResult.results as PragmaForeignKey[]).reduce<DrizzleTable['foreignKeys']>((acc, fk) => {
      ...
      for (const idx of indexResult.results as PragmaIndexInfo[]) {
        ...
        const indexColumns = (indexInfoResult.results as { name: string }[]).map(c => c.name);
    ...
    logWarning(`Introspection failed: ${error instanceof Error ? error.message : String(error)}`, {
      module: 'drizzle',
      operation: 'introspect',
      databaseId
    });
 ...
 */
export function validateDrizzleSchema(schema: string): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
    ...
    if (currentTable) {
      ...
      for (const col of currentTable.columns) {
        if (!targetColNames.has(col.name)) {
          changes.push(`- Drop column: ${currentTable.name}.${col.name} (requires table rebuild)`);
        }
      }
    }
  }
...
export async function getMigrationStatus(
  ...
  try {
    ...
    return {
      ...
      appliedMigrations: migrations.results as { id: number; hash: string; created_at: string }[]
    };
  } catch (error) {
    logWarning(`Failed to get migration status: ${error instanceof Error ? error.message : String(error)}`, {
      module: 'drizzle',
