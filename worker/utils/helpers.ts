...
export function validateQuery(query: string): {
  ...
  if (upperQuery.startsWith("DROP ")) {
    return {
      valid: false,
      warning: "DROP operations require explicit confirmation",
    };
  }
  ...
  if (upperQuery.startsWith("DELETE ") || upperQuery.startsWith("UPDATE ")) {
    if (!upperQuery.includes(" WHERE ")) {
      return {
        valid: false,
        warning:
          "DELETE/UPDATE without WHERE clause requires explicit confirmation",
      };
    }
  }
...
export function buildWhereClause(
  ...
  for (const [columnName, filter] of filterEntries) {
    ...
    // Handle IN operator
      ...
      const escapedValues = limitedValues
