...
export async function processMonitoring(env: Env): Promise<void> {
  ...
  try {
    ...
    const sentinel = sentinels && sentinels.length > 0 ? sentinels[0] : null;
    ...
    // Let's use a 1-hour window to catch recent spikes, except storage which is absolute.
    const end = new Date();
    ...
    // Reset sentinel on success
    if (sentinel && sentinel.enabled && sentinel.consecutive_failures > 0) {
      await env.METADATA.prepare(
        `UPDATE monitoring_thresholds SET consecutive_failures = 0, last_alert_at = datetime('now'), updated_at = datetime('now') WHERE id = ?`,
      ).bind(sentinel.id).run();
    }

    // 4. Evaluate each database threshold
...
function extractMetricValue(account: D1AnalyticsResult["viewer"]["accounts"][0], threshold: MonitoringThreshold): number | null {
  ...
  switch (threshold.metric_type) {
    case "storage_usage": {
      if (!threshold.storage_limit_bytes) return null;
    ...
    case "error_rate": {
      ...
      for (const q of queryStats) {
        ...
        if (q.avg?.queryDurationMs && q.avg.queryDurationMs > 10000) {
          failed += q.count || 0;
        }
      }
