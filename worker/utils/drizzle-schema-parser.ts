...
function parseColumnModifiers(columnDef: string): Partial<ParsedColumn> {
  ...
  const refMatch = /\.references\s*\(\s*\(\s*\)\s*=>\s*(\w+)\.(\w+)/.exec(
    columnDef,
  );
...
function parseTableColumns(tableBody: string): ParsedColumn[] {
  ...
  for (const def of columnDefs) {
    ...
    const match =
      /^(\w+)\s*:\s*((?:integer|text|real|blob|numeric|int)\s*\(.+)$/i.exec(
        def,
      );
...
export function parseDrizzleSchema(schemaContent: string): ParsedSchema {
  ...
  try {
    logInfo("Parsing Drizzle schema", {
...
export function compareSchemas(
  ...
  for (const uploadedTable of uploadedSchema) {
    ...
    for (const uploadedCol of uploadedTable.columns) {
      if (!currentColNames.has(uploadedCol.name)) {
        ...
        if (uploadedCol.isPrimaryKey) {
          warnings.push(
            `Cannot add PRIMARY KEY column '${uploadedCol.name}' to existing table '${uploadedTable.name}' - requires table rebuild`,
          );
          continue;
        }
        ...
        if (uploadedCol.isNotNull && uploadedCol.defaultValue === null) {
          warnings.push(
            `Column '${uploadedCol.name}' is NOT NULL without default - adding with default NULL instead`,
          );
        } else {
    ...
    for (const currentCol of currentTable.columns) {
      if (!uploadedColNames.has(currentCol.name)) {
        ...
        warnings.push(
          `Column '${currentTable.name}.${currentCol.name}' will be dropped`,
        );
      }
    }
    ...
    for (const uploadedCol of uploadedTable.columns) {
      ...
      if (currentCol.type !== uploadedCol.type) {
        warnings.push(
          `Column '${uploadedTable.name}.${uploadedCol.name}' type change from ${currentCol.type} to ${uploadedCol.type} requires table rebuild`,
        );
      }
      ...
      if (currentCol.isNotNull !== uploadedCol.isNotNull) {
        warnings.push(
          `Column '${uploadedTable.name}.${uploadedCol.name}' NOT NULL constraint change requires table rebuild`,
        );
      }
    }
  }
  ...
  const droppedTables = differences.filter(
    (d) => d.type === "table_drop",
  ).length;
  const addedCols = differences.filter((d) => d.type === "column_add").length;
  const droppedCols = differences.filter(
    (d) => d.type === "column_drop",
  ).length;
