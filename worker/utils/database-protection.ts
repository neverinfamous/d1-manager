...
const PROTECTED_NAMES = ["d1-manager-metadata", "kv-manager-metadata"];
 ...
 */
export function createProtectedDatabaseResponse(
  ...
  return new Response(
    JSON.stringify({
      ...
      message:
        "This database is used by system applications and cannot be accessed.",
    }),
    {
      status: 403,
      headers,
...
export async function getDatabaseInfo(
  ...
  try {
    ...
    logWarning(
      `Failed to get database info: ${err instanceof Error ? err.message : String(err)}`,
      {
        module: "protection",
