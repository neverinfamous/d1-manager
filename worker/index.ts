...
async function handleApiRequest(
  ...
  if (!url.pathname.startsWith("/api/")) {
    ...
    if (!env.ASSETS) {
      ...
      throw new Error(
        "ASSETS binding is not configured. Check your wrangler.toml configuration.",
      );
    }
  ...
  if (
    url.pathname === "/api/databases/colors" ||
  ...
  if (url.pathname.startsWith("/api/tables/")) {
    return await handleTableRoutes(
      request,
      env,
      url,
      corsHeaders,
  ...
  if (url.pathname.startsWith("/api/query/")) {
    ...
    return await handleQueryRoutes(
      request,
      env,
      url,
      corsHeaders,
  ...
  if (url.pathname.startsWith("/api/saved-queries")) {
    return await handleSavedQueriesRoutes(
      request,
      env,
      url,
      corsHeaders,
  ...
  if (url.pathname.startsWith("/api/indexes/")) {
    return await handleIndexRoutes(
      request,
      env,
      url,
      corsHeaders,
  ...
  if (url.pathname.startsWith("/api/time-travel/")) {
    const timeTravelResponse = await handleTimeTravelRoutes(
      request,
      env,
      url,
      corsHeaders,
  ...
  if (
    ...
    const r2BackupResponse = await handleR2BackupRoutes(
      request,
      env,
      url,
      corsHeaders,
  ...
  if (url.pathname.startsWith("/api/scheduled-backups")) {
    const scheduledBackupResponse = await handleScheduledBackupRoutes(
      request,
      env,
      url,
      corsHeaders,
  ...
  if (url.pathname.startsWith("/api/drizzle/")) {
    return await handleDrizzleRoutes(
      request,
      env,
      url,
      corsHeaders,
  ...
  try {
    ...
    logWarning(
      `Error serving asset: ${err instanceof Error ? err.message : String(err)}`,
      {
        module: "worker",
