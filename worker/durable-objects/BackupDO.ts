...
import {
  triggerWebhooks,
  createBackupCompletePayload,
  createRestoreCompletePayload,
} from "../utils/webhooks";
...
export class BackupDO {
      ...
      try {
        ...
        switch (jobType) {
          case "database-backup":
            await this.processDatabaseBackup(
              body as R2BackupParams & { jobId: string },
            );
            break;
          case "table-backup":
            await this.processTableBackup(
              body as R2TableBackupParams & { jobId: string },
            );
            break;
          case "database-restore":
            await this.processDatabaseRestore(
  ...
  private async processDatabaseBackup(
    ...
    try {
      ...
      const exportStartData =
        (await exportResponse.json()) as ExportStartResult;

      if (!exportStartData.success) {
        const errorMsg =
          exportStartData.errors?.[0]?.message ?? "Export failed";
      ...
      let signedUrl =
        exportStartData.result?.signed_url ??
        exportStartData.result?.result?.signed_url;
      ...
      if (!signedUrl && bookmark) {
        ...
        while (!signedUrl && attempts < maxAttempts) {
          ...
          if (!pollResponse.ok) {
            ...
            throw new Error(
              `Poll failed: ${pollResponse.statusText} - ${errorText}`,
            );
          }
          ...
          const pollSignedUrl =
            pollData.result?.signed_url ?? pollData.result?.result?.signed_url;
          if (pollSignedUrl) {
          ...
          await updateJobProgress(db, {
            jobId,
            processedItems: Math.round(progress),
            totalItems: 100,
          });
        }
        ...
        if (!signedUrl) {
          throw new Error(
            "Export timeout after 6 minutes - database export is taking longer than expected. Try a manual backup or contact support if the issue persists.",
          );
        }
      }
      ...
      if (!downloadResponse.ok) {
        throw new Error(
          `Failed to download export: ${downloadResponse.statusText}`,
        );
      }
   ...
   */
  private async processTableBackup(
    params: R2TableBackupParams & { jobId: string },
  ): Promise<void> {
    const {
      jobId,
      databaseId,
    ...
    try {
      ...
      if (!schemaResponse.ok) {
        throw new Error(
          `Failed to get table schema: ${schemaResponse.statusText}`,
        );
      }
      ...
      const schemaData =
        (await schemaResponse.json()) as QueryResult<ColumnInfo>;
      ...
      const tableData = (await dataResponse.json()) as QueryResult<
      ...
      if (format === "json") {
        ...
        const insertStatements = rows.map((row) =>
          buildInsertStatement(tableName, columns, row),
        );
   ...
   */
  private async processDatabaseRestore(
    ...
    try {
      ...
      if (!initResponse.ok || !initData.success) {
        const errorMsg =
          initData.errors?.[0]?.message ?? initResponse.statusText;
      ...
      if (!ingestResponse.ok || !ingestData.success) {
        const errorMsg =
          ingestData.errors?.[0]?.message ?? ingestResponse.statusText;
