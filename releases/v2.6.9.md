...
- **Fixed Query Console**: Resolved a blank page issue caused by `Prism is not defined` in production builds.
- **Dependency Updates**: Updated 9 dependencies including `eslint`, `vite`, `wrangler` and `@cloudflare/workers-types` to their latest versions.
