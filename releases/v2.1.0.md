...
- **D1→R2 Connector Pattern**: Export database content as markdown for AI indexing
...
### ⚠️ FTS5 Warning for Scheduled Backups
...
### Dependencies
- **ESLint Group**: `eslint` 9.36.0 → 9.39.2, `eslint-plugin-react-refresh` 0.4.14 → 0.4.26, `typescript-eslint` 8.50.1 → 8.52.0, `globals` 16.4.0 → 17.0.0
- **Types Group**: `@types/node` 24.9.1 → 24.9.3, `@types/react` 19.2.2 → 19.2.3, `@types/react-dom` 19.2.2 → 19.2.3
- **sql-formatter**: 15.6.10 → 15.6.12
...
- **React Version Mismatch**: Fixed React error #527 by aligning `react-dom` (19.2.1 → 19.2.3) to match `react` version
...
- **[Wiki](https://github.com/neverinfamous/d1-manager/wiki)** — Updated guides including new AI Search page
- **[Webhooks](https://github.com/neverinfamous/d1-manager/wiki/Webhooks)** — Complete rewrite for 13-event engine
