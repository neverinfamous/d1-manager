...
import { Zap, RefreshCw, Loader2, Copy, Check, AlertCircle, TrendingUp, Database, Sparkles, PlayCircle, Cloud, AlertTriangle } from 'lucide-react';
...
import { analyzeIndexes, createIndex, getR2BackupStatus, backupToR2, type IndexRecommendation, type IndexAnalysisResult, type R2BackupStatus, type R2BackupSource } from '@/services/api';
...
export function IndexAnalyzer({ databaseId, databaseName }: IndexAnalyzerProps): React.JSX.Element {
  ...
  const handleCreateIndex = async (recommendation: IndexRecommendation): Promise<void> => {
    ...
    try {
      ...
      const indexNameMatch = /CREATE\s+INDEX\s+(?:IF\s+NOT\s+EXISTS\s+)?(\w+)/i.exec(recommendation.suggestedSQL);
      const indexName = indexNameMatch?.[1];
      
      // Determine columns (could be single or composite)
      const columns = recommendation.indexType === 'composite' && recommendation.compositeColumns
  ...
  const handleCreateAllIndexes = async (): Promise<void> => {
    ...
    for (let i = 0; i < recommendations.length; i++) {
      ...
      try {
        const indexNameMatch = /CREATE\s+INDEX\s+(?:IF\s+NOT\s+EXISTS\s+)?(\w+)/i.exec(rec.suggestedSQL);
        const extractedName = indexNameMatch?.[1];
        
        const columns = rec.indexType === 'composite' && rec.compositeColumns
  ...
  const getPriorityIcon = (priority: 'high' | 'medium' | 'low'): React.JSX.Element => {
    switch (priority) {
      case 'high':
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={() => void loadAnalysis(true)} disabled={loading}>
            {loading ? (
      ...
      <Dialog open={showCreateAllDialog} onOpenChange={() => !creatingAll && resetCreateAllDialog()}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              This will create {analysis?.recommendations.length ?? 0} indexes on your database.
            </DialogDescription>
          </DialogHeader>
          ...
          {!createAllProgress && (
            <div className="space-y-4">
              <div className="flex items-start gap-3 p-3 bg-yellow-50 dark:bg-yellow-950/30 border border-yellow-200 dark:border-yellow-900 rounded-lg">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-yellow-700 dark:text-yellow-300 mt-1">
                    While indexes improve query performance, they also increase storage usage and can affect write performance. 
              ...
              {/* Backup Options */}
              {r2BackupStatus?.configured && r2BackupStatus?.bucketAvailable && (
                <div className="space-y-2">
                  <p className="text-sm font-medium">Optional: Create a backup first</p>
                  <Button
                    variant="outline"
                    size="sm"
                ...
                <div className="max-h-40 overflow-y-auto border rounded-lg p-2 bg-muted/30">
                  {analysis?.recommendations.map((rec, idx) => (
                    <div key={idx} className="flex items-center gap-2 py-1 text-sm">
                      <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${getPriorityColor(rec.priority)}`}>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span>Progress</span>
                  <span>{createAllProgress.current} / {createAllProgress.total}</span>
                </div>
                <Progress value={(createAllProgress.current / createAllProgress.total) * 100} />
                  ...
                  {createAllProgress.succeeded.length > 0 && (
                    <div className="p-3 bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 rounded-lg">
                      <div className="flex items-center gap-2 text-green-700 dark:text-green-300 font-medium text-sm">
                        ...
                        {createAllProgress.succeeded.length} indexes created successfully
                      </div>
                    </div>
                  )}
          ...
          <DialogFooter>
            {!createAllProgress ? (
              <>
                ...
                <Button onClick={() => void handleCreateAllIndexes()} disabled={creatingAll}>
                  {creatingAll && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}
                  Create All Indexes
                </Button>
              </>
            ) : createAllProgress.current === createAllProgress.total ? (
              <Button onClick={resetCreateAllDialog}>
                Done
              </Button>
            ) : (
