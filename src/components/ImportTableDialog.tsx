...
export function ImportTableDialog({
  ...
  const handleImport = async (): Promise<void> => {
    ...
    try {
      ...
      const tableExists = currentTables.some(
        (t) => t.name.toLowerCase() === finalTableName.toLowerCase(),
      );

      // Validate table name for new tables
      if (createNewTable && tableExists) {
        throw new Error(
          `Table "${finalTableName}" already exists. Please choose a different name or uncheck "Create new table" to import into the existing table.`,
        );
      }
      ...
      if (!createNewTable && format !== "sql" && !tableExists) {
        throw new Error(
          `Table "${finalTableName}" does not exist. Check "Create new table" to create it first.`,
        );
      }
  ...
  const hasContent =
    ...
    (source === "paste" && pastedContent.trim() !== "");
  const hasValidTarget = createNewTable
    ? newTableName.trim() !== ""
    : targetTable !== "";
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => {
                  setCsvHasHeader(checked === true);
                  setPreview(null);
                }}
              />
              <Label htmlFor="csv-header" className="font-normal">
          ...
          {!createNewTable && format !== "sql" && (
            <fieldset className="grid gap-3">
              ...
              <RadioGroup
                ...
                onValueChange={(v) =>
                  setDuplicateHandling(v as "fail" | "replace" | "ignore")
                }
              >
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="fail" id="dup-fail" />
                  <Label htmlFor="dup-fail" className="font-normal">
                    Fail (stop on first duplicate)
                  </Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="replace" id="dup-replace" />
                  <Label htmlFor="dup-replace" className="font-normal">
                    Replace (update existing rows)
                  </Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="ignore" id="dup-ignore" />
                  <Label htmlFor="dup-ignore" className="font-normal">
                    Skip (ignore duplicate rows)
                  </Label>
                </div>
              </RadioGroup>
            </fieldset>
          ...
          <fieldset className="grid gap-3">
            ...
            >
              ...
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="source-paste" className="font-normal">
                  Paste content
                </Label>
              </div>
            </RadioGroup>
              ...
              {!createNewTable && format !== "sql" && (
                <div className="grid gap-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Column names in your data must match the existing table
                    columns exactly.
                  </p>
                </div>
              )}
                ...
                <div className="text-xs font-mono bg-background p-2 rounded overflow-x-auto">
                  ...
                  {preview.rowCount > 3 && (
                    <div className="text-muted-foreground">
                      ... and {preview.rowCount - 3} more rows
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    The table is missing {columnMismatch.missingColumns.length}{" "}
                    column
                    {columnMismatch.missingColumns.length !== 1 ? "s" : ""} from
                    your import data:
                  </p>
                  <div className="text-xs font-mono bg-background p-2 rounded">
                    <p>
                      <strong>Missing:</strong>{" "}
                      {columnMismatch.missingColumns.join(", ")}
                    </p>
                    <p className="mt-1 text-muted-foreground">
                      <strong>Table has:</strong>{" "}
                      {columnMismatch.tableColumns.join(", ")}
                    </p>
                    <p className="text-muted-foreground">
                      <strong>Import has:</strong>{" "}
                      {columnMismatch.importColumns.join(", ")}
                    </p>
                  </div>
                </div>
              </div>
