...
export function FTS5PerformanceMetrics({ searchResponse }: FTS5PerformanceMetricsProps) {
  ...
  const getRecommendations = () => {
    ...
    if (executionTime > 500) {
      recommendations.push('Query is slow. Consider adding a prefix index or using more specific terms.');
    }
    
    if (getEfficiency() < 1) {
      recommendations.push('Low efficiency. Try narrowing your search with additional terms or column filters.');
    }
    
    if (total > 1000 && results.length === total) {
      recommendations.push('Large result set. Consider using pagination or more specific search terms.');
    }
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-2xl font-bold ${getPerformanceColor(executionTime)}`}>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Rows Scanned</p>
                <p className="text-2xl font-bold">
                  {meta?.rowsScanned ? meta.rowsScanned.toLocaleString() : 'N/A'}
                </p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-2xl font-bold ${getEfficiencyColor(efficiency)}`}>
                  {efficiency}%
                </p>
              </div>
