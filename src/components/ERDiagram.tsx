...
import {
  exportERDiagramAsPNG,
  exportERDiagramAsJSON,
} from "@/services/erExport";
      ...
      <div className="p-2 max-h-[300px] overflow-y-auto">
        {data.columns.length === 0 ? (
          ...
          data.columns.map((col, index) => (
            <div
              key={index}
              className="text-xs py-1 px-2 hover:bg-muted rounded flex items-center justify-between gap-2"
            >
              <div className="flex items-center gap-2 min-w-0 flex-1">
                {col.pk && (
                  <span className="text-yellow-500 flex-shrink-0">🔑</span>
                )}
                {!col.pk && col.isForeignKey && (
...
function ERDiagramContent({
  ...
  const [enrichedColumns, setEnrichedColumns] = useState<
    Record<string, ExtendedColumnInfo[]>
  >({});
  ...
  const loadDiagramData = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to load diagram data",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div
      className={`relative ${isFullscreen ? "fixed inset-0 z-50 bg-background w-screen h-screen" : "h-[calc(100vh-220px)]"}`}
    >
      <ReactFlow
        nodes={nodes}
        edges={edges}
      ...
      >
        ...
        <Panel
          position="top-left"
          className="bg-background/95 backdrop-blur-sm border rounded-lg p-3 shadow-lg space-y-2"
        >
