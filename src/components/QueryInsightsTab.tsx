...
export function QueryInsightsTab({
  ...
  const [expandedQueries, setExpandedQueries] = useState(new Set<string>());
        ...
        <CardContent className="py-12">
          <div className="text-center text-muted-foreground">
            ...
            <p className="text-sm mt-1">
              Query insights will appear here once queries are executed against
              your databases.
            </p>
          </div>
        </CardContent>
    ...
    <div className="space-y-4">
            ...
            <div className="text-2xl font-bold text-red-600 dark:text-red-400">
              {
                queryInsights.filter(
                  (q) => getPerformanceLevel(q.avgTimeMs) === "critical",
                ).length
              }
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">
              {
                queryInsights.filter(
                  (q) => getPerformanceLevel(q.avgTimeMs) === "warning",
                ).length
              }
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600 dark:text-green-400">
              {
                queryInsights.filter(
                  (q) => getPerformanceLevel(q.avgTimeMs) === "good",
                ).length
              }
            </div>
            <p className="text-xs text-muted-foreground">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Top {sortedInsights.length} queries by{" "}
            {sortField === "totalTime"
                    ...
                    <div className="flex items-center gap-4 flex-shrink-0">
                      <div className="text-right">
                        <div className="text-sm font-medium">
                          {formatLatency(insight.totalTimeMs)}
                        </div>
                      ...
                      <div className="text-right">
                        <div className="text-sm font-medium">
                          {formatLatency(insight.avgTimeMs)}
                        </div>
                        <div className="text-xs text-muted-foreground">avg</div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm font-medium">
                          {formatNumber(insight.executionCount)}
                        </div>
                        <div className="text-xs text-muted-foreground">
                          calls
                        </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        <div>
                          ...
                          <span className="font-medium">
                            {insight.databaseName ??
                              insight.databaseId.slice(0, 8)}
                          </span>
                        </div>
                        <div>
                          <span className="text-muted-foreground flex items-center gap-1">
                        ...
                        <div>
                          ...
                          <span className="font-medium block">
                            {formatNumber(insight.rowsWritten)}
                          </span>
                        </div>
                      </div>
                    </div>
