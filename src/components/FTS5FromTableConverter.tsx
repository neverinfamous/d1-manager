...
export function FTS5FromTableConverter({
  ...
  const validateAndConvert = async (): Promise<void> => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to convert table to FTS5",
      );
    } finally {
      setConverting(false);
    }
  ...
  const generateSQL = (): string => {
    ...
    lines.push(
      `CREATE VIRTUAL TABLE ${ftsTableName} USING fts5(${columnNames}, ${options});`,
    );
    lines.push("");
    ...
    if (externalContent) {
      lines.push(
        `INSERT INTO ${ftsTableName}(rowid, ${columnNames}) SELECT rowid, ${columnNames} FROM ${sourceTable};`,
      );
    } else {
      lines.push(
        `INSERT INTO ${ftsTableName}(${columnNames}) SELECT ${columnNames} FROM ${sourceTable};`,
      );
    }
    ...
    if (externalContent && createTriggers) {
      ...
      lines.push(
        `  INSERT INTO ${ftsTableName}(rowid, ${columnNames}) VALUES (NEW.rowid, ${selectedColumns.map((c) => `NEW.${c}`).join(", ")});`,
      );
      lines.push("END;");
    }
      ...
      <DialogContent className="max-h-[90vh] overflow-hidden w-[95vw] max-w-[1100px]">
        ...
        <div className="space-y-6 py-4 overflow-y-auto max-h-[calc(90vh-180px)]">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              disabled={converting || loadingTables}
            >
            ...
            <div className="flex items-start space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  setExternalContent(checked === true)
                }
                disabled={converting}
              />
              <div className="flex-1">
                <Label
                  htmlFor="external-content"
                  className="font-medium cursor-pointer"
                >
                  Use external content table
                </Label>
                <p className="text-sm text-muted-foreground mt-1">
                  Store data in original table to save space. FTS5 will
            ...
            {externalContent && (
              <div className="ml-6 flex items-start space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setCreateTriggers(checked === true)
                  }
                  disabled={converting}
                />
                <div className="flex-1">
                  <Label
                    htmlFor="create-triggers"
                    className="font-normal cursor-pointer"
                  >
                    Create sync triggers
                  </Label>
                  <p className="text-xs text-muted-foreground mt-1">
                    Automatically update FTS5 when source table changes
          ...
          <div className="space-y-2">
            ...
            <div className="bg-muted rounded-lg p-4 overflow-hidden">
              <pre
                className="text-xs overflow-x-auto max-h-64 whitespace-pre-wrap break-words"
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={
              converting || !sourceTable || selectedColumns.length === 0
            }
          >
