...
export function DiffEditor({
  ...
  const getSourceText = useCallback(
    (source: DiffSource, customText: string): string => {
  ...
  const leftText = useMemo(
    () => getSourceText(leftSource, leftCustom),
    [getSourceText, leftSource, leftCustom],
  );
  const rightText = useMemo(
    () => getSourceText(rightSource, rightCustom),
    [getSourceText, rightSource, rightCustom],
  );
  ...
  const renderDiff = useMemo(() => {
    const leftLines: {
      content: string;
      type: "unchanged" | "removed" | "spacer";
      lineNum: number | null;
    }[] = [];
    const rightLines: {
      content: string;
      type: "unchanged" | "added" | "spacer";
      lineNum: number | null;
    }[] = [];
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="grid grid-cols-[1fr,auto,1fr] gap-4 items-end">
              ...
              <Select
                value={
                  leftSource.type === "saved"
                    ? `saved:${String(leftSource.savedQueryId ?? "")}`
                    : leftSource.type
                }
                onValueChange={(value) => {
                  if (value === "current") {
                    ...
                    setLeftSource({
                      type: "saved",
                      savedQueryId: parseInt(value.replace("saved:", ""), 10),
                    });
                  }
                }}
              >
            ...
            <div className="space-y-2">
              ...
              <Select
                value={
                  rightSource.type === "saved"
                    ? `saved:${String(rightSource.savedQueryId ?? "")}`
                    : rightSource.type
                }
                onValueChange={(value) => {
                  if (value === "current") {
          ...
          {(leftSource.type === "custom" || rightSource.type === "custom") && (
            <div className="grid grid-cols-2 gap-4">
              ...
              {rightSource.type === "custom" && (
                <div
                  className={`space-y-2 ${leftSource.type !== "custom" ? "col-start-2" : ""}`}
                >
      ...
      <p className="text-xs text-muted-foreground text-center">
        Compare your current query with saved queries or paste custom SQL to see
        differences.
        <span className="mx-2">•</span>
        <span className="text-green-600 dark:text-green-400">Green</span> =
        additions, <span className="text-red-600 dark:text-red-400">Red</span> =
        deletions
      </p>
    </div>
  );
