...
export function ExportDatabaseDialog({
    ...
    const loadTables = async (): Promise<void> => {
        ...
        try {
            ...
            const exportableTables = tables.filter(t => t.type === 'table' || t.type === 'virtual');
    ...
    const getFormatIcon = (f: ExportFormat): React.ReactNode => {
        switch (f) {
            case 'sql': return <FileText className="h-5 w-5" />;
            case 'json': return <FileJson className="h-5 w-5" />;
            case 'csv': return <Table2 className="h-5 w-5" />;
        }
    };
    ...
    const getProgressPercent = (): number => {
        ...
        const tableProgress = (progress.tablesCompleted / progress.totalTables) * 85;
        ...
        <Dialog open={open} onOpenChange={(isOpen) => !isOpen && handleClose()}>
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Export &quot;{database.name}&quot; for backup or cross-account migration
                    </DialogDescription>
                </DialogHeader>
                    ...
                    {step === 'format' && (
                        <>
                            <fieldset className="space-y-3">
                                <legend className="text-sm font-medium leading-none mb-3">Export Format</legend>
                                <RadioGroup value={format} onValueChange={(v) => setFormat(v as ExportFormat)}>
                                    <div className="flex items-start space-x-3 p-3 border rounded-lg hover:bg-muted/50 cursor-pointer">
                                        <RadioGroupItem value="sql" id="format-sql" className="mt-0.5" />
                                        <div className="flex-1">
                                            <Label htmlFor="format-sql" className="font-medium flex items-center gap-2 cursor-pointer">
                                                <FileText className="h-4 w-4 text-blue-600" />
                                                SQL Dump
                                            </Label>
                                            <p className="text-xs text-muted-foreground mt-1">
                                                Complete SQL file with CREATE TABLE and INSERT statements.
                                                Best for <code className="bg-muted px-1 rounded">wrangler d1 execute</code>.
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-start space-x-3 p-3 border rounded-lg hover:bg-muted/50 cursor-pointer">
                                        ...
                                        <div className="flex-1">
                                            <Label htmlFor="format-json" className="font-medium flex items-center gap-2 cursor-pointer">
                                    ...
                                    <div className="flex items-start space-x-3 p-3 border rounded-lg hover:bg-muted/50 cursor-pointer">
                                        ...
                                        <div className="flex-1">
                                            <Label htmlFor="format-csv" className="font-medium flex items-center gap-2 cursor-pointer">
                                                <Table2 className="h-4 w-4 text-orange-600" />
                                                CSV (ZIP)
                                            </Label>
                                            <p className="text-xs text-muted-foreground mt-1">
                                                ZIP file with one CSV per table plus metadata.
                                                Best for spreadsheet tools and data analysis.
                                            </p>
                                        </div>
                                    </div>
                                </RadioGroup>
                            ...
                            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-900 rounded-lg p-3">
                                <div className="flex items-start gap-2">
                                    ...
                                    <div className="text-xs text-blue-700 dark:text-blue-300">
                                        <span className="font-medium">Cross-Account Migration:</span> Download this export and
                                        import it into another D1 Manager deployment or use <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded">wrangler d1 execute</code>.
                                    </div>
                                </div>
                            </div>
                        </>
                    ...
                    {step === 'tables' && (
                        <>
                            <div className="space-y-3">
                                        ...
                                        {availableTables.map(table => (
                                            <div key={table.name} className="flex items-center gap-2">
                                                <Checkbox
                                                    ...
                                                    onCheckedChange={(checked) => handleTableToggle(table.name, checked === true)}
                                                />
                                                <Label htmlFor={`table-${table.name}`} className="font-normal flex items-center gap-2 cursor-pointer">
                                                    {table.name}
                                                    {table.type === 'virtual' && (
                                ...
                                {hasFTS5 && selectedTables.some(t => availableTables.find(at => at.name === t)?.type === 'virtual') && (
                                    <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900 rounded-lg p-3">
                                        <div className="flex items-start gap-2">
                                            ...
                                            <div className="text-xs text-amber-700 dark:text-amber-300">
                                                <span className="font-medium">FTS5 Tables:</span> Virtual table schemas cannot be fully exported.
                                                The export will include a comment placeholder.
                                            </div>
                                        </div>
                                    </div>
                                )}
                        ...
                        <div className="space-y-4">
                            <div className="space-y-3">
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    />
                                    <div>
                                        <Label htmlFor="include-data" className="font-medium cursor-pointer">Include Data</Label>
                                        <p className="text-xs text-muted-foreground">
                                    ...
                                    >
                                        {ROW_LIMIT_OPTIONS.map(opt => (
                                            <div key={opt.value} className="flex items-center space-x-2">
                                                <RadioGroupItem value={String(opt.value)} id={`limit-${String(opt.value)}`} />
                                                <Label htmlFor={`limit-${String(opt.value)}`} className="text-sm font-normal cursor-pointer">
                                                    {opt.label}
                                                </Label>
                                            </div>
                                        ))}
                            ...
                            {completed && (
                                <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-900 rounded-lg p-4">
                                    ...
                                    <p className="text-sm text-green-700 dark:text-green-300 mt-2">
                                        ...
                                        {format === 'json' && ' The file includes schema metadata and typed data.'}
                                        {format === 'csv' && ' Extract the ZIP and import CSVs as needed.'}
                                    </p>
                                    {progress?.warning && (
