...
const STRICT_COLUMN_TYPES = ["INTEGER", "INT", "REAL", "TEXT", "BLOB", "ANY"];

export function SchemaDesigner({
  open,
  onOpenChange,
  onCreateTable,
}: SchemaDesignerProps): React.JSX.Element {
  ...
  const validateAndCreate = async (): Promise<void> => {
    ...
    for (const col of columns) {
      ...
      if (col.isGenerated) {
        const exprValidation = validateGeneratedExpression(
          col.generatedExpression,
        );
        if (!exprValidation.isValid) {
          setError(
            `Column "${col.name}": ${exprValidation.error ?? "Invalid expression"}`,
          );
          return;
        }
      }
      ...
      if (col.isGenerated && col.primaryKey) {
        setError(
          `Generated column "${col.name}" cannot be a primary key. Generated columns are computed values.`,
        );
        return;
      }
      ...
      if (strictMode) {
        ...
        if (col.isGenerated) {
          setError(
            `STRICT mode tables cannot have generated columns. Disable STRICT mode or remove the generated column "${col.name}".`,
          );
          return;
        }
        ...
        if (!STRICT_COLUMN_TYPES.includes(col.type)) {
          setError(
            `Column "${col.name}" has type "${col.type}" which is not valid for STRICT mode. Use one of: ${STRICT_COLUMN_TYPES.join(", ")}.`,
          );
          return;
        }
      }
    ...
    if (duplicates.length > 0) {
      setError(
        `Duplicate column name: "${duplicates[0] ?? ""}". Each column must have a unique name.`,
      );
      return;
    }
    ...
    if (!hasPrimaryKey) {
      setError(
        "Table must have at least one primary key column. Select a column to be the primary key.",
      );
      return;
    }
          ...
          <div className="bg-blue-50 dark:bg-blue-950/50 border border-blue-200 dark:border-blue-800 rounded-lg p-4 space-y-3">
            <div className="flex items-start gap-3">
                  ...
                  if (e.target.checked) {
                    setColumns((cols) =>
                      cols.map((col) => ({
                        ...
                        type: STRICT_COLUMN_TYPES.includes(col.type)
                          ? col.type
                          : col.type === "BOOLEAN" || col.type === "NUMERIC"
                            ? "INTEGER"
                            : col.type === "DATE" ||
                                col.type === "DATETIME" ||
                                col.type === "JSON"
              ...
              <div className="flex-1">
                <label
                  htmlFor="strict-mode"
                  className="flex items-center gap-2 font-medium text-blue-900 dark:text-blue-100 cursor-pointer"
                >
                  <Shield className="h-4 w-4" />
                  Enable STRICT Mode
                </label>
                <p className="text-xs text-blue-700 dark:text-blue-300 mt-1">
                  Enforces type checking - values must match declared column
                  types. Only allows INTEGER, INT, REAL, TEXT, BLOB, ANY types.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-2 pt-2 border-t border-blue-200 dark:border-blue-700">
              ...
              <p className="text-xs text-blue-700 dark:text-blue-300">
                <strong>Need full-text search?</strong> Use the{" "}
                <strong>FTS5 Manager</strong> tab to create FTS5 virtual tables
                for powerful text search capabilities.
              </p>
            </div>
          </div>
