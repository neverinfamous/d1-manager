...
export function TableDependenciesView({
  tableName,
  dependencies,
}: TableDependenciesViewProps): React.JSX.Element | null {
  ...
  const getCascadeColor = (action: string | null): string => {
    ...
    if (upper === "RESTRICT" || upper === "NO ACTION")
      return "text-red-600 dark:text-red-500";
    if (upper === "SET NULL" || upper === "SET DEFAULT")
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2 ml-6">
            {dependencies.outbound.map((dep, idx) => (
              ...
              >
                <AlertTriangle
                  className={`h-4 w-4 mt-0.5 flex-shrink-0 ${getCascadeColor(dep.onDelete)}`}
                />
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    ...
                    <span className={getCascadeColor(dep.onDelete)}>
                      {formatCascadeAction(dep.onDelete)}
                    </span>
                    {dep.rowCount > 0 && (
                      <span>
                        {" "}
                        • {dep.rowCount.toLocaleString()} row
                        {dep.rowCount !== 1 ? "s" : ""} in this table
                      </span>
                    )}
                  </div>
                </div>
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    ...
                    {dep.rowCount > 0 &&
                      dep.onDelete?.toUpperCase() === "CASCADE" && (
                        <span className={getCascadeColor(dep.onDelete)}>
                          {" "}
                          • Deleting will cascade to{" "}
                          {dep.rowCount.toLocaleString()} row
                          {dep.rowCount !== 1 ? "s" : ""}
                        </span>
                      )}
                    {dep.rowCount > 0 &&
                      dep.onDelete?.toUpperCase() !== "CASCADE" && (
                        <span>
                          {" "}
                          • {dep.rowCount.toLocaleString()} dependent row
                          {dep.rowCount !== 1 ? "s" : ""}
                        </span>
                      )}
                  </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm">
              <p className="font-medium text-yellow-800 dark:text-yellow-200">
                Deleting <span className="font-mono">{tableName}</span> will
                affect {dependencies.inbound.length} dependent table
                {dependencies.inbound.length !== 1 ? "s" : ""}
              </p>
              <p className="text-yellow-700 dark:text-yellow-300 mt-1">
                {dependencies.inbound.some(
                  (d) => d.onDelete?.toUpperCase() === "CASCADE",
                ) && (
                  <span>
                    Some rows will be automatically deleted due to CASCADE
                    constraints.
                  </span>
                )}
                {dependencies.inbound.some(
                  (d) =>
                    d.onDelete?.toUpperCase() === "RESTRICT" ||
                    d.onDelete?.toUpperCase() === "NO ACTION",
                ) && (
                  <span>
                    Some foreign key constraints may prevent deletion
                    (RESTRICT/NO ACTION).
                  </span>
                )}
              </p>
