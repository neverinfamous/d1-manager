...
export const SQL_DOCS: Record<string, SqlDoc> = {
  ...
  // Data Types
  TYPE: {
    description: 'Specifies the data type of a column, or refers to object type in sqlite_master',
...
export function getSqlDoc(keyword: string): SqlDoc | undefined {
  ...
  const compounds = ['ORDER BY', 'GROUP BY', 'INNER JOIN', 'LEFT JOIN', 'RIGHT JOIN', 'CROSS JOIN', 'UNION ALL'];
