 ...
 */
function getCurrentWord(sql: string, cursorPos: number): { word: string; start: number } {
  let start = cursorPos;
  ...
  while (start > 0) {
    ...
    if (!char || WORD_DELIMITERS.has(char)) {
      ...
      if (char === '.') {
        ...
        while (start > 0 && prevChar !== undefined && !WORD_DELIMITERS.has(prevChar)) {
          start--;
        }
      }
      break;
...
export function extractTablesFromQuery(sql: string): string[] {
  ...
  while ((aliasMatch = aliasPattern.exec(sql)) !== null) {
    ...
    if (tableName && !tables.includes(tableName) && !isKeyword(tableName.toUpperCase())) {
      tables.push(tableName);
    }
  }
...
export function parseContext(sql: string, cursorPos: number): SqlContext {
  ...
  if ((lastOrderBy > lastFrom && lastOrderBy > lastSelect) ||
