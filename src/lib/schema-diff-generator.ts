...
export function compareFullSchemas(
  ...
  for (const tableName of allTableNames) {
    ...
    if (leftTable && !rightTable) {
      ...
      const hasColumnChanges = columnDiffs.some(
...
function compareForeignKeys(
  leftFks: ForeignKeyInfo[],
  rightFks: ForeignKeyInfo[],
): ForeignKeyDiff[] {
  const fkKey = (fk: ForeignKeyInfo): string =>
  ...
  for (const key of allFkKeys) {
    ...
    if (leftFk && !rightFk) {
      ...
      if (
        leftFk.onDelete !== rightFk.onDelete ||
        leftFk.onUpdate !== rightFk.onUpdate
      ) {
...
function compareTriggers(
  leftTriggers: TriggerInfo[],
  rightTriggers: TriggerInfo[],
): TriggerDiff[] {
  ...
  for (const diff of diffs) {
    switch (diff.status) {
      ...
      case "modified":
        ...
        if (diff.columnDiffs) {
          steps.push(
            ...generateColumnMigrationSteps(
              diff.table,
              diff.columnDiffs,
              diff.tableInfo?.right,
            ),
          );
        }
        ...
        if (diff.indexDiffs) {
          steps.push(
            ...generateIndexMigrationSteps(diff.table, diff.indexDiffs),
          );
        }
        ...
        if (diff.triggerDiffs) {
          steps.push(
            ...generateTriggerMigrationSteps(diff.table, diff.triggerDiffs),
          );
        }
        break;
    }
...
function generateColumnMigrationSteps(
  ...
  const hasComplexChanges = columnDiffs.some(
    (d) => d.status === "removed" || d.status === "modified",
  );
...
function generateTableRecreationStep(
  tableName: string,
  targetTable: TableWithColumns,
): MigrationStep {
  const tempTableName = `${tableName}_migration_temp`;
  const columnDefs = targetTable.columns
    .map((col) => {
      let def = `"${col.name}" ${col.type || "TEXT"}`;
      if (col.pk > 0) def += " PRIMARY KEY";
      if (col.notnull && col.pk === 0) def += " NOT NULL";
...
function generateIndexMigrationSteps(
  tableName: string,
  indexDiffs: IndexDiff[],
): MigrationStep[] {
  const steps: MigrationStep[] = [];
...
function generateFKMigrationSteps(
  tableName: string,
  fkDiffs: ForeignKeyDiff[],
): MigrationStep[] {
  const steps: MigrationStep[] = [];
...
function generateTriggerMigrationSteps(
  tableName: string,
  triggerDiffs: TriggerDiff[],
): MigrationStep[] {
  const steps: MigrationStep[] = [];
