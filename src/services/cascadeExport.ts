...
export class CascadeExportService {
  ...
  public exportAsCSV(): string {
    ...
    // Header
    lines.push(
      "Table Name,Action,Rows Before,Rows After,Affected Rows,Depth,Severity",
    );
    ...
    lines.push("Summary");
    lines.push(
      `Total Tables Affected,${String(this.simulation.affectedTables.length)}`,
    );
    lines.push(
      `Total Rows Affected,${String(this.simulation.totalAffectedRows)}`,
    );
    lines.push(`Maximum Cascade Depth,${String(this.simulation.maxDepth)}`);
    lines.push(
      `Total Cascade Paths,${String(this.simulation.cascadePaths.length)}`,
    );
  ...
  public exportAsText(): string {
    ...
    for (let depth = 0; depth <= stats.maxDepth; depth++) {
      ...
      if (tables.length > 0) {
        ...
        for (const table of tables) {
          const affectedRows = table.rowsBefore - table.rowsAfter;
          lines.push(
            `  - ${table.tableName} (${table.action}): ${String(affectedRows)} row(s) affected`,
          );
        }
      }
    }
    ...
    if (this.simulation.warnings.length > 0) {
      ...
      for (const warning of this.simulation.warnings) {
        ...
        lines.push(
          `${icon} [${warning.severity.toUpperCase()}] ${warning.message}`,
        );
      }
      lines.push("");
    }
  ...
  public async exportAsPDF(graphElement: HTMLElement | null): Promise<Blob> {
    ...
    if (this.simulation.whereClause) {
      pdf.text(
        `WHERE Clause: ${this.simulation.whereClause}`,
        margin,
        yPosition,
      );
      yPosition += 6;
