...
async function parseErrorResponse(
  ...
  if (response.status === 503) {
    return {
      error: "Service temporarily unavailable (503). Please try again.",
    };
  }
...
class APIService {
  ...
  private handleResponse(response: Response): Response {
    ...
    if (response.status === 429) {
      throw new Error(
        "Rate limit exceeded. Cloudflare limits API requests - please wait a moment and try again.",
      );
    }

    if (response.status === 503) {
      throw new Error(
        "Service temporarily unavailable. Cloudflare may be experiencing issues - please try again shortly.",
      );
    }

    if (response.status === 504) {
      throw new Error(
        "Request timed out. The database may be under heavy load - please try again.",
      );
    }
  ...
  async getDatabaseInfo(
    ...
    const response = await fetch(
      `${WORKER_API}/api/databases/${databaseId}/info`,
      {
        credentials: "include",
      },
    );
   ...
   */
  async setReadReplication(
    databaseId: string,
    mode: ReadReplicationMode,
  ): Promise<D1Database> {
    const response = await fetch(
      `${WORKER_API}/api/databases/${databaseId}/replication`,
      {
        method: "PUT",
        headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        error.error ??
          error.message ??
          `Failed to set read replication: ${response.statusText}`,
      );
    }
  ...
  async renameDatabase(
    ...
    try {
      ...
      const response = await fetch(
        `${WORKER_API}/api/databases/${databaseId}/rename`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        ...
        const errorMessage =
          ...
          error.error ??
          error.message ??
          `Rename failed: ${response.statusText}`;
      ...
      const data = (await response.json()) as {
        result: D1Database & { oldId: string };
        success: boolean;
      };
  ...
  async optimizeDatabases(
    ...
    if (!response.ok) {
      ...
      throw new Error(
        error.message ??
          error.error ??
          `Failed to optimize databases: ${String(response.status)}`,
      );
    }
    ...
    try {
      ...
      if (exportData.skipped?.length) {
        ...
        if (skipped?.reason === "fts5") {
          throw new Error(
            `Cannot clone database with FTS5 tables: ${skipped.details?.join(", ") ?? "FTS5 tables detected"}`,
          );
        }
        if (skipped) {
      ...
      if (!createResponse.ok) {
        ...
        throw new Error(
          error.message ??
            error.error ??
            `Failed to create database: ${String(createResponse.status)}`,
        );
      }
  ...
  async listTables(
    ...
    const response = await fetch(
      `${WORKER_API}/api/tables/${databaseId}/list`,
      {
        credentials: "include",
      },
    );
  ...
  > {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to get table foreign keys: ${response.statusText}`,
      );
    }
  ...
  async getTableDependencies(
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to get table dependencies: ${response.statusText}`,
      );
    }

    const data =
      (await response.json()) as ApiResponse<TableDependenciesResponse>;
    const result = data.result ?? {};
  ...
  async renameTable(
    ...
    if (!response.ok) {
      const error = await parseErrorResponse(response);
      throw new Error(
        error.error ?? `Failed to rename table: ${response.statusText}`,
      );
    }
  ...
  async addColumn(
    ...
    if (!response.ok) {
      const error = await parseErrorResponse(response);
      throw new Error(
        error.error ?? `Failed to add column: ${response.statusText}`,
      );
    }
  ...
  async renameColumn(
    ...
    if (!response.ok) {
      const error = await parseErrorResponse(response);
      throw new Error(
        error.error ?? `Failed to rename column: ${response.statusText}`,
      );
    }
  ...
  async modifyColumn(
    ...
    if (!response.ok) {
      const error = await parseErrorResponse(response);
      throw new Error(
        error.error ?? `Failed to modify column: ${response.statusText}`,
      );
    }
  ...
  async deleteColumn(
    ...
    if (!response.ok) {
      const error = await parseErrorResponse(response);
      throw new Error(
        error.error ?? `Failed to delete column: ${response.statusText}`,
      );
    }
  ...
  async cloneTable(
    ...
    if (!response.ok) {
      const error = await parseErrorResponse(response);
      throw new Error(
        error.error ?? `Failed to clone table: ${response.statusText}`,
      );
    }
  ...
  async cloneTables(
    ...
    succeeded: { oldName: string; newName: string }[];
    failed: { name: string; error: string }[];
  }> {
   ...
   */
  async checkStrictCompatibility(
    databaseId: string,
    tableName: string,
  ): Promise<{
    compatible: boolean;
    ...
    if (!response.ok) {
      ...
      throw new Error(
        error.error ??
          `Failed to check STRICT compatibility: ${response.statusText}`,
      );
    }
  ...
  async convertToStrict(
    ...
    if (!response.ok) {
      ...
      throw new Error(
        error.error ??
          `Failed to convert table to STRICT: ${response.statusText}`,
      );
    }
  ...
  async exportTables(
    ...
    try {
      ...
      for (let i = 0; i < tableNames.length; i++) {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to export table ${tableName}: ${response.statusText}`,
          );
        }
  ...
  async executeQuery<T = Record<string, unknown>>(
    ...
    const response = await fetch(
      `${WORKER_API}/api/query/${databaseId}/execute`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        errorMessage =
          "Request blocked by security rules. If you're testing SQL injection, this is expected - Cloudflare WAF is protecting your database.";
        ...
        try {
          ...
          if (error.error) {
            ...
            errorMessage =
              d1Error?.message ?? d1Error?.error ?? "Unknown D1 error";
  ...
  async executeBatch(
    ...
    const response = await fetch(
      `${WORKER_API}/api/query/${databaseId}/batch`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  async updateDatabaseColor(
    ...
    if (!response.ok) {
      const data = (await parseErrorResponse(response)) as ApiErrorResponse & {
      ...
      if (data.requiresUpgrade) {
        throw new Error(
          "Database colors feature requires a schema upgrade. See the upgrade instructions in the README.",
        );
      }
      ...
      throw new Error(
        data.message ??
          data.error ??
          `Failed to update color: ${String(response.status)}`,
      );
    }
  }
   ...
   */
  async getTableColors(
    databaseId: string,
  ): Promise<Record<string, DatabaseColor>> {
    const response = await fetch(
  ...
  async updateTableColor(
    ...
    if (!response.ok) {
      ...
      if (data.requiresUpgrade) {
        throw new Error(
          "Table colors feature requires a schema upgrade. See the upgrade instructions in the README.",
        );
      }
      ...
      throw new Error(
        data.message ??
          data.error ??
          `Failed to update table color: ${String(response.status)}`,
      );
    }
  }
}
...
export const listTables = (
  databaseId: string,
  skipCache = false,
): Promise<TableInfo[]> => api.listTables(databaseId, skipCache);
export const getTableSchema = (
  databaseId: string,
  tableName: string,
  skipCache = false,
): Promise<ColumnInfo[]> =>
  api.getTableSchema(databaseId, tableName, skipCache);
export const getTableForeignKeys = (
  databaseId: string,
  tableName: string,
  skipCache = false,
): Promise<
...
> => api.getTableForeignKeys(databaseId, tableName, skipCache);
 ...
 */
export const getFullDatabaseSchema = async (
  databaseId: string,
): Promise<FullDatabaseSchema> => {
  const WORKER_API = import.meta.env.VITE_WORKER_API || window.location.origin;

  const response = await fetch(
    `${WORKER_API}/api/tables/${databaseId}/schema-full`,
    {
      method: "GET",
      credentials: "include",
    },
...
export const createDatabase = (
  name: string,
  location?: string,
): Promise<D1Database> => api.createDatabase(name, location);
...
export const getSavedQueries = async (
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch saved queries: ${String(response.status)}`,
    );
  }
...
export const createSavedQuery = async (
  ...
  if (!response.ok) {
    const error = (await parseErrorResponse(response)) as {
      error?: string;
      message?: string;
    };
    throw new Error(
      error.message ??
        error.error ??
        `Failed to save query: ${String(response.status)}`,
    );
  }
...
export const updateSavedQuery = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/saved-queries/${String(id)}`,
    {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to update query: ${String(response.status)}`,
    );
  }
...
export const deleteSavedQuery = async (id: number): Promise<void> => {
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to delete query: ${String(response.status)}`,
    );
  }
};
...
export const getUndoHistory = async (
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to get undo history: ${String(response.status)}`,
    );
  }
...
export const restoreUndo = async (
  databaseId: string,
  undoId: number,
): Promise<string> => {
  const response = await fetch(
    `${WORKER_API}/api/undo/${databaseId}/restore/${String(undoId)}`,
    {
      method: "POST",
      credentials: "include",
    },
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.details ??
        error.error ??
        `Failed to restore: ${String(response.status)}`,
    );
  }
...
export const clearUndoHistory = async (databaseId: string): Promise<number> => {
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to clear undo history: ${String(response.status)}`,
    );
  }
...
export const deleteRowsWithUndo = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/tables/${databaseId}/${encodeURIComponent(tableName)}/rows/delete`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to delete rows: ${String(response.status)}`,
    );
  }
...
export const simulateCascadeImpact = async (
  ...
  // Check cache first (unless skipCache is true)
  if (!skipCache) {
    const cached = getCachedCascadeSimulation(
      databaseId,
      targetTable,
      whereClause,
    );
  ...
  const response = await fetch(
    `${WORKER_API}/api/tables/${databaseId}/simulate-cascade`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to simulate cascade: ${String(response.status)}`,
    );
  }
...
function getCachedFkData(
  databaseId: string,
  includeCycles: boolean,
  includeSchemas: boolean,
): ForeignKeyGraphWithCycles | null {
...
function setCachedFkData(
  databaseId: string,
  includeCycles: boolean,
  includeSchemas: boolean,
  data: ForeignKeyGraphWithCycles,
): void {
  const key = getFkCacheKey(databaseId, includeCycles, includeSchemas);
...
export const getAllForeignKeys = async (
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    if (!response.ok) {
      const error = await parseErrorResponse(response);
      throw new Error(
        error.error ?? `Failed to get foreign keys: ${String(response.status)}`,
      );
    }

    const data = (await response.json()) as {
      result: ForeignKeyGraphWithCycles;
      success: boolean;
    };
  ...
  throw (
    lastError ?? new Error("Failed to get foreign keys after multiple retries")
  );
};
 ...
 */
export const getCircularDependencies = async (
  databaseId: string,
): Promise<CircularDependencyCycle[]> => {
  ...
  const response = await fetch(
    `${WORKER_API}/api/tables/${databaseId}/circular-dependencies`,
    {
      method: "GET",
      credentials: "include",
    },
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to get circular dependencies: ${String(response.status)}`,
    );
  }

  const data = (await response.json()) as {
    result: CircularDependencyCycle[];
    success: boolean;
  };
  return data.result;
};
...
export const simulateAddForeignKey = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/tables/${databaseId}/foreign-keys/simulate`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to simulate foreign key: ${String(response.status)}`,
    );
  }

  const data = (await response.json()) as {
    result: SimulateForeignKeyResult;
    success: boolean;
  };
  return data.result;
};
...
export const addForeignKey = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/tables/${databaseId}/foreign-keys/add`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.message ??
        error.error ??
        `Failed to add foreign key: ${String(response.status)}`,
    );
  }
...
export const modifyForeignKey = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/tables/${databaseId}/foreign-keys/${encodeURIComponent(constraintName)}`,
    {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.message ??
        error.error ??
        `Failed to modify foreign key: ${String(response.status)}`,
    );
  }
...
export const deleteForeignKey = async (
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.message ??
        error.error ??
        `Failed to delete foreign key: ${String(response.status)}`,
    );
  }
 ...
 */
export const listFTS5Tables = async (
  databaseId: string,
  skipCache = false,
): Promise<FTS5TableInfo[]> => {
  // Check cache first
  if (!skipCache) {
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to list FTS5 tables: ${String(response.status)}`,
    );
  }

  const data = (await response.json()) as {
    result: FTS5TableInfo[];
    success: boolean;
  };
...
export const createFTS5Table = async (
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to create FTS5 table: ${String(response.status)}`,
    );
  }

  const data = (await response.json()) as {
    result: { tableName: string; created: boolean };
    success: boolean;
  };
...
export const createFTS5FromTable = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/fts5/${databaseId}/create-from-table`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to create FTS5 from table: ${String(response.status)}`,
    );
  }
...
export const getFTS5Config = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/fts5/${databaseId}/${encodeURIComponent(tableName)}/config`,
    {
      method: "GET",
      credentials: "include",
    },
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to get FTS5 config: ${String(response.status)}`,
    );
  }

  const data = (await response.json()) as {
    result: Partial<FTS5TableConfig>;
    success: boolean;
  };
  return data.result;
};
...
export const deleteFTS5Table = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/fts5/${databaseId}/${encodeURIComponent(tableName)}`,
    {
      method: "DELETE",
      credentials: "include",
    },
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to delete FTS5 table: ${String(response.status)}`,
    );
  }
...
export const convertFTS5ToTable = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/fts5/${databaseId}/${encodeURIComponent(ftsTableName)}/convert-to-table`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to convert FTS5 to table: ${String(response.status)}`,
    );
  }
...
export const rebuildFTS5Index = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/fts5/${databaseId}/${encodeURIComponent(tableName)}/rebuild`,
    {
      method: "POST",
      credentials: "include",
    },
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to rebuild FTS5 index: ${String(response.status)}`,
    );
  }
...
export const optimizeFTS5 = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/fts5/${databaseId}/${encodeURIComponent(tableName)}/optimize`,
    {
      method: "POST",
      credentials: "include",
    },
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to optimize FTS5 index: ${String(response.status)}`,
    );
  }

  const data = (await response.json()) as {
    result: { optimized: boolean };
    success: boolean;
  };
  return data.result;
};
...
export const searchFTS5 = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/fts5/${databaseId}/${encodeURIComponent(tableName)}/search`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to search FTS5 table: ${String(response.status)}`,
    );
  }

  const data = (await response.json()) as {
    result: FTS5SearchResponse;
    success: boolean;
  };
  return data.result;
};
...
export const getFTS5Stats = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/fts5/${databaseId}/${encodeURIComponent(tableName)}/stats`,
    {
      method: "GET",
      credentials: "include",
    },
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to get FTS5 stats: ${String(response.status)}`,
    );
  }
...
const indexAnalysisCache = new Map<
  string,
  { data: IndexAnalysisResult; timestamp: number }
>();
...
const fts5Cache = new Map<
  string,
  { data: FTS5TableInfo[]; timestamp: number }
>();
...
const databaseInfoCache = new Map<
  string,
  { data: D1Database; timestamp: number }
>();
...
const timeTravelCache = new Map<
  string,
  { data: TimeTravelCacheData; timestamp: number }
>();
...
function setTimeTravelCache(
  databaseId: string,
  data: Partial<TimeTravelCacheData>,
): void {
  ...
  const merged: TimeTravelCacheData = {
    ...
    bookmarkLoaded:
      "bookmarkLoaded" in data
        ? (data.bookmarkLoaded ?? false)
        : (existing?.bookmarkLoaded ?? false),
    history:
      "history" in data ? (data.history ?? []) : (existing?.history ?? []),
    historyLoaded:
      "historyLoaded" in data
        ? (data.historyLoaded ?? false)
        : (existing?.historyLoaded ?? false),
  };
...
export const analyzeIndexes = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/indexes/${databaseId}/analyze`,
    {
      method: "GET",
      credentials: "include",
    },
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to analyze indexes: ${String(response.status)}`,
    );
  }

  const data = (await response.json()) as IndexAnalysisResult & {
    success: boolean;
  };
  const result = {
...
export const createIndex = async (
  ...
  const response = await fetch(
    `${WORKER_API}/api/indexes/${databaseId}/create`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.message ??
        error.error ??
        `Failed to create index: ${String(response.status)}`,
    );
  }
...
export const getJobList = async (options?: {
  ...
  const response = await fetch(`${WORKER_API}/api/jobs?${params.toString()}`, {
    method: "GET",
    credentials: "include",
    cache: "no-store",
  });
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to get job list: ${String(response.status)}`,
    );
  }
...
export const getJobEvents = async (
  ...
  const response = await fetch(`${WORKER_API}/api/jobs/${jobId}/events`, {
    method: "GET",
    credentials: "include",
    cache: "no-store",
  });
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to get job events: ${String(response.status)}`,
    );
  }
...
export const getJobStatus = async (jobId: string): Promise<JobListItem> => {
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to get job status: ${String(response.status)}`,
    );
  }
 ...
 */
export const getCurrentBookmark = async (
  databaseId: string,
  skipCache = false,
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to get bookmark: ${String(response.status)}`,
    );
  }
...
export const getBookmarkHistory = async (
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to get bookmark history: ${String(response.status)}`,
    );
  }
...
export const captureBookmark = async (
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to capture bookmark: ${String(response.status)}`,
    );
  }
...
export const deleteBookmarkEntry = async (
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to delete bookmark: ${String(response.status)}`,
    );
  }
 ...
 */
export const generateRestoreCommand = (
  databaseName: string,
  bookmark: string,
): string => {
 ...
 */
export const getR2BackupStatus = async (
  skipCache = false,
): Promise<R2BackupStatus> => {
  // Check cache first (10 min TTL - rarely changes)
  if (
    !skipCache &&
    r2BackupStatusCache &&
    Date.now() - r2BackupStatusCache.timestamp < R2_STATUS_CACHE_TTL
  ) {
    return r2BackupStatusCache.data;
  }

  const response = await fetch(`${WORKER_API}/api/r2-backup/status`, {
    credentials: "include",
  });
 ...
 */
export const listR2Backups = async (
  databaseId: string,
): Promise<R2BackupListItem[]> => {
  const response = await fetch(
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to list R2 backups: ${String(response.status)}`,
    );
  }

  const data = (await response.json()) as {
    result: R2BackupListItem[];
    success: boolean;
  };
  return data.result;
};
 ...
 */
export const listOrphanedR2Backups = async (): Promise<
  OrphanedBackupGroup[]
> => {
  const response = await fetch(`${WORKER_API}/api/r2-backup/orphaned`, {
    credentials: "include",
  });
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to list orphaned backups: ${String(response.status)}`,
    );
  }

  const data = (await response.json()) as {
    result: OrphanedBackupGroup[];
    success: boolean;
  };
  return data.result;
};
 ...
 */
export const deleteAllR2Backups = async (
  databaseId: string,
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to delete backups: ${String(response.status)}`,
    );
  }
...
export const backupToR2 = async (
  ...
  const response = await fetch(`${WORKER_API}/api/r2-backup/${databaseId}`, {
    method: "POST",
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to start R2 backup: ${String(response.status)}`,
    );
  }

  const data = (await response.json()) as {
    result: R2BackupJobResponse;
    success: boolean;
  };
  return data.result;
};
...
export const backupTableToR2 = async (
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to start table backup: ${String(response.status)}`,
    );
  }
...
export const restoreFromR2 = async (
  ...
  const response = await fetch(`${WORKER_API}/api/r2-restore/${databaseId}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to start restore: ${String(response.status)}`,
    );
  }
...
export const deleteR2Backup = async (
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to delete backup: ${String(response.status)}`,
    );
  }
};
...
export const bulkDeleteR2Backups = async (
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to bulk delete backups: ${String(response.status)}`,
    );
  }
...
export const downloadR2Backup = async (
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to download backup: ${String(response.status)}`,
    );
  }
...
export const getR2BackupSourceLabel = (source: R2BackupSource): string => {
  switch (source) {
    ...
    case "strict_mode":
      return "Before STRICT Mode";
    case "fts5_convert":
      return "Before FTS5 Convert";
    case "column_modify":
      return "Before Column Modify";
    case "table_export":
      return "Table Export";
    case "table_backup":
      return "Table Backup";
...
export const listScheduledBackups = async (): Promise<ScheduledBackup[]> => {
  const response = await fetch(`${WORKER_API}/api/scheduled-backups`, {
    credentials: "include",
  });
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to list scheduled backups: ${String(response.status)}`,
    );
  }
 ...
 */
export const getScheduledBackup = async (
  databaseId: string,
): Promise<ScheduledBackup | null> => {
  const response = await fetch(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to get scheduled backup: ${String(response.status)}`,
    );
  }
 ...
 */
export const saveScheduledBackup = async (
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to save scheduled backup: ${String(response.status)}`,
    );
  }
 ...
 */
export const deleteScheduledBackup = async (
  databaseId: string,
): Promise<void> => {
  const response = await fetch(
    `${WORKER_API}/api/scheduled-backups/${databaseId}`,
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to delete scheduled backup: ${String(response.status)}`,
    );
  }
};
 ...
 */
export const toggleScheduledBackup = async (
  databaseId: string,
): Promise<ScheduledBackup> => {
  const response = await fetch(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to toggle scheduled backup: ${String(response.status)}`,
    );
  }
...
export const getMigrationStatus = async (): Promise<MigrationStatus> => {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to get migration status: ${String(response.status)}`,
    );
  }
...
export const applyMigrations = async (): Promise<MigrationResult> => {
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to apply migrations: ${String(response.status)}`,
    );
  }
...
export const markLegacyMigrations = async (
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ??
        `Failed to mark legacy migrations: ${String(response.status)}`,
    );
  }
...
function getCachedCascadeSimulation(
  databaseId: string,
  tableName: string,
  whereClause?: string,
): CascadeSimulationResult | null {
...
function setCachedCascadeSimulation(
  databaseId: string,
  tableName: string,
...
function getCachedTableDependencies(
  databaseId: string,
  tableNames: string[],
): TableDependenciesResponse | null {
...
function setCachedTableDependencies(
  databaseId: string,
  tableNames: string[],
  data: TableDependenciesResponse,
): void {
...
function getCachedTableFK(
  databaseId: string,
  tableName: string,
): TableFKCacheEntry["data"] | null {
...
function setCachedTableFK(
  databaseId: string,
  tableName: string,
  data: TableFKCacheEntry["data"],
): void {
  const key = getTableFKCacheKey(databaseId, tableName);
 ...
 * Call after FK modifications
 */
export function invalidateTableFKCache(
  databaseId?: string,
  tableName?: string,
): void {
  if (databaseId && tableName) {
...
let r2BackupStatusCache: { data: R2BackupStatus; timestamp: number } | null =
  null;
...
function getCachedDatabaseList(): D1Database[] | null {
  if (
    databaseListCache &&
    Date.now() - databaseListCache.timestamp < DATABASE_LIST_CACHE_TTL
  ) {
...
export const getMetrics = async (
  ...
  if (!response.ok) {
    const error = await parseErrorResponse(response);
    throw new Error(
      error.error ?? `Failed to get metrics: ${String(response.status)}`,
    );
  }
