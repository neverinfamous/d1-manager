...
export function detectFormat(
    ...
    try {
      ...
      if (parsed["meta"] && parsed["tables"]) {
        return {
          format: "json",
          confidence: "high",
          reason: "D1 Manager JSON export format",
        };
      }
      return {
        format: "json",
  ...
  return {
    format: "sql",
    confidence: "low",
    reason: "Unable to detect format, assuming SQL",
  };
}
...
export function parseJsonExport(content: string): ImportPreview {
  ...
  if (data.meta?.format !== "d1-manager-export") {
    warnings.push(
      "File does not appear to be a D1 Manager export. Import may fail.",
    );
  }
  ...
  for (const [tableName, tableData] of Object.entries(data.tables)) {
    ...
    if (tableData.type === "virtual") {
      warnings.push(
        `Table "${tableName}" is a virtual table (FTS5) - schema may need manual recreation.`,
      );
    }
  }
...
export function convertJsonToSql(
  ...
  for (const [tableName, tableData] of Object.entries(data.tables)) {
    ...
    if (tableData.data.length > 0) {
      lines.push(
        `-- Data for ${tableName} (${String(tableData.rowCount)} rows)`,
      );
      ...
      if (firstRow) {
        ...
        for (const row of tableData.data) {
          const values = columnNames
            ...
            .join(", ");
          lines.push(
            `INSERT INTO "${tableName}" (${quotedColumns}) VALUES (${values});`,
          );
        }
        lines.push("");
      }
...
export async function parseCsvZipExport(file: File): Promise<ImportPreview> {
  ...
  if (metadataFile) {
    try {
      ...
      warnings.push(
        "Could not parse metadata file. Table schemas may be incomplete.",
      );
    }
  } else {
    warnings.push(
      "No metadata file found. Table schemas will be inferred from CSV headers.",
    );
  }
  ...
  if (metadata) {
    ...
    const emptyTables = metadata.tables.filter(
      (t) => !csvTableNames.has(t.name),
    );
    ...
    if (emptyTables.length > 0) {
      warnings.push(
        `${String(emptyTables.length)} table(s) have no data and will only have schema created.`,
      );
    }
  }
 ...
 */
export async function convertCsvZipToSql(
  file: File,
  options: {
    includeSchema?: boolean;
    deferForeignKeys?: boolean;
  ...
  for (const csvName of csvFiles) {
    ...
    if (dataLines.length > 0) {
      ...
      for (const dataLine of dataLines) {
        ...
        lines.push(
          `INSERT INTO "${tableName}" (${quotedColumns}) VALUES (${sqlValues});`,
        );
      }
      lines.push("");
    }
  ...
  if (includeSchema && metadata) {
    ...
    const emptyTables = metadata.tables.filter(
      (t) => !csvTableNames.has(t.name) && t.type !== "virtual",
    );
...
export function validateSqlContent(sql: string): {
  ...
  if (/DROP\s+TABLE/i.test(sql)) {
    warnings.push(
      "SQL contains DROP TABLE statements. Existing tables may be deleted.",
    );
  }
  if (/DELETE\s+FROM/i.test(sql) && !/WHERE/i.test(sql)) {
    warnings.push(
      "SQL contains DELETE statements without WHERE clause. All rows may be deleted.",
    );
  }
