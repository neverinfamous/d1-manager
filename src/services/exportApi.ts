...
export async function exportDatabase(
  ...
  for (let i = 0; i < tablesToExport.length; i++) {
    ...
    if (options.includeSchema) {
      tableExport.schema = await withRetry(() =>
        getTableSchema(databaseId, table.name),
      );
    ...
    if (options.includeData && table.type !== "virtual") {
      ...
      const result = await withRetry(() =>
        getTableData(databaseId, table.name, options.rowLimit),
      );
  ...
  const allTablesEmpty =
    emptyTableCount === tableExports.length && tableExports.length > 0;
  ...
  onProgress?.({
    ...
    ...(allTablesEmpty
      ? {
          warning:
            "All selected tables are empty. For CSV format, only metadata will be included.",
        }
      : {}),
  });
...
function generateSqlDump(
  ...
  for (const table of result.tables) {
    if (table.data.length > 0) {
      lines.push(
        `-- Data for table: ${table.name} (${String(table.rowCount)} rows)`,
      );
      ...
      if (firstRow) {
        ...
        for (const row of table.data) {
          const values = columns
            ...
            .join(", ");
          lines.push(
            `INSERT INTO "${table.name}" (${quotedColumns}) VALUES (${values});`,
          );
        }
        lines.push("");
      }
...
function generateCsvContent(
  ...
  const columns =
    schema.length > 0 ? schema.map((c) => c.name) : Object.keys(data[0] ?? {});
